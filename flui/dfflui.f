      SUBROUTINE DFFLUI( KNOMOB , NERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    DEFINIR LES CARACTERISTIQUES PHYSIQUES DES FLUIDES
C -----    ET LES FORCES EXERCEES
C
C ENTREE :
C --------
C KNOMOB : NOM DE L'OBJET A FLUIDER
C
C SORTIE :
C --------
C NERR   : CODE D'ERREUR =0 SI PAS DE PROBLEME ,=1 SINON
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C AUTEUR : ALAIN PERRONNET ANALYSE NUMERIQUE UPMC PARIS    DECEMBRE 1998
C AUTEURS: BENHAMADOUCHE   BOYER ZACHARIE                  JANVIER  2000
C AUTEUR : ALAIN PERRONNET LJLL UPMC PARIS                 NOVEMBRE 2008
C AUTEUR : ALAIN PERRONNET LJLL UPMC & St Pierre du Perray    AVRIL 2010
C2345X7..............................................................012
      PARAMETER        (LLIST1=10, LLIST2=5)
      ...
      CHARACTER*10   LISTE2(LLIST2)
      DATA LISTE1 / 'MASSE','VISCOSITE',
     %              'FORCE','VITFLUIN','PREFLUIN',
     %              'NotDefined','NotDefined',
     %              'COEFPRESSION','VITESSEANGULAIRE','COEFBOUSSINESQ' /
      DATA LISTE2 / 'FORCE','BLVITESSE','BLPRESSION',
     %              'VITFLUIN','PREFLUIN' /
C
C     VERIFICATIONS
C     =============
...
C     S'IL N'EXISTE PAS RETOUR A LA DEMANDE DU NOM DE L'OBJET
      IF( NTDFOB .LE. 0 ) THEN
         NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) ='ERREUR : TMS XYZSOMMET INCONNU OBJET '// KNOMOB
         ELSE
            KERR(1) ='ERROR : TMS XYZSOMMET UNKNOWN for OBJECT '//KNOMOB
         ENDIF
         CALL LEREUR
         GOTO 9900
      ENDIF
...
C
C     LE TABLEAU TOPOLOGIE DE L'OBJET EST RECHERCHE
      CALL LXTSOU( NTLXOB , 'TOPOLOGIE' , NTTOPO , MNTOPO )
      IF( NTTOPO .LE. 0 ) THEN
         NBLGRC(NRERR) = 3
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'ERREUR: OBJET ' // KNOMOB
            KERR(2) = 'SANS TOPOLOGIE DEFINIE'
            KERR(3) = 'RELANCER MAILLER pour FAIRE INTERPOLATION'
         ELSE
            KERR(1) = 'ERROR: OBJECT ' // KNOMOB
            KERR(2) = 'WITHOUT TMS TOPOLOGIE'
            KERR(3) = 'EXECUTE AGAIN MAILLER to DO INTERPOLATION'
         ENDIF
         CALL LEREUR
         GOTO 9900
      ENDIF
...
C
      IF(NBOBIN.GT.1) THEN
        NBLGRC(NRERR) = 1
        IF( LANGAG .EQ. 0 ) THEN
           KERR(1) = 'TRAITER PLUSIEURS SURFACES N''EST PAS PROGRAMME'
        ELSE
           KERR(1) = 'TREATMENT of ONLY ONE SURFACE IS PROGRAMMED'
        ENDIF
        CALL LEREUR
        GOTO 9900
      ENDIF
...
C        LE TYPE DE L'OBJET
         ...
         IF ( NTOB .LE. 0 ) THEN
            NBLGRC(NRERR) = 2
            IF( LANGAG .EQ. 0 ) THEN
        KERR(1) = 'CONDITION LIMITE: POINT ou LIGNE ou SURFACE INCONNU '
     %                // KNOM
            ELSE
       KERR(1) = 'BOUNDARY CONDITION: UNKNOWN POINT or LINE or SURFACE '
     %                // KNOM
            ENDIF
            KERR(2) = 'TYPE: ' // KNM
            CALL LEREUR
...
C        IMPRESSION D'UN MESSAGE
         ...
         L3 = INDEX( KHIST(1)(1:6),'LIGNE' )
         IF( L3 .GT. 0 ) THEN
C           RECTIFICATION POUR L'ANGLAIS
            KHIST(1)(1:1) = 'L'
            KHIST(1)(2:2) = 'I'
            KHIST(1)(3:3) = 'N'
            KHIST(1)(4:4) = 'E'
...
C
            IF( IERR .NE. 0 .OR. MCN(MNTS+WTBLVI) .EQ. 0 ) THEN
C               LE TYPE EST NUL => DESTRUCTION DU TABLEAU
                ...
                ELSE
                   KERR(2) = 'BOUNDARY CONDITION DELETED'
                ENDIF
                CALL LERESU
                GOTO 210
            ENDIF
