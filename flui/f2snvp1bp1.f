      SUBROUTINE F2SNVP1BP1( NEF00,  NTYEF,   NBEF,   NUSOTR,
     %                       MOARET, MXARET,  MNLARE,
     %                       DT,     CoefTr,  XYZPOI, NDDLNO,
     %                       NTDLHB, VXYZPN0, VITMAX0,
     %                       BE,     IERR )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : CALCUL DU VECTEUR TRANSPORT NAVIER STOKES DU TRIANGLE BREZZI FORTIN
...
C       de  Integrale CoefTr tP1B ui(tn,X(tn;tn+1,x) dx
C
C ENTREES:	
C --------
...
C NUSOTR : NUMERO DES 3 SOMMETS DES NBEF TRIANGLES
...
C SORTIES:
C --------
C BE     : VECTEUR ELEMENTAIRE DE 4+4+3=11 COEFFICIENTS
C          LES 3 DERNIERS COEFFICIENTS BE(9:11) SONT NULS
...
C
      INTEGER            NEF00, NTYEF, NBEF, MOARET, MXARET, MNLARE,
     %                   NTDLHB, IERR
      REAL               DT, XYZPOI(3,*)
      INTEGER            NUSOTR(NBEF,3)
      INTEGER            NDDLNO(0:*)
      DOUBLE PRECISION   VXYZPN0(1:NTDLHB), VITMAX0
      DOUBLE PRECISION   BE(11)
C
      DOUBLE PRECISION   Xe(2,3), DELTAe1, DELTAe
      DOUBLE PRECISION   CoefTr, RDPOIDS, PRCBL1(4), PRCBL0(4),
     %                   D, CB1, BULLE, V(2), VA(2), XRCBL, YRCBL,
     %                   XEBL0, YEBL0, XEBL1, YEBL1, X1, Y1, VIT0MX
...
C     --------------------------------------------------------------------------
      CALL SPTP1BP12D( DT,      XYZPOI,  NEF00,  NBEF,  NUSOTR,
     %                 NDDLNO,  NTDLHB,
...
C     CALCUL DU JACOBIEN DE Fe POLYNOMIALE DE DEGRE 1 de NEF00
     ...
     %             - (Xe(1,3) - X1) * (Xe(2,2) - Y1) )
      IF( DELTAe1 .LE. 0D0 ) THEN
         IF( LANGAG .EQ. 0 ) THEN
            WRITE(IMPRIM,*) 'f2snvp1bp1: ATTENTION EF',NEF00,
     %                    ' de SURFACE*2=',DELTAe1,' NON PRIS EN COMPTE'
         ELSE
            WRITE(IMPRIM,*) 'f2snvp1bp1: ATTENTION FE',NEF00,
...
C        INITIALISATION DE LA VITESSE ET DE L'EF INITIAL DE RECHERCHE
         ...
         NEF  = NEF00
C
C        RECUPERATION DES COORDONNEES Xe(3,2) DES SOMMETS DU TRIANGLE NEF
         DO I=1,3
            NS = NUSOTR(NEF,I)
            DO K=1,2
               Xe(K,I) = DBLE( XYZPOI( K, NS ) )
...
C        COORDONNEES de (XEBL1,YEBL1)=Fe(bl) SUR LE TRIANGLE COURANT a tn+1
         ...
         XEBL1 = CB1 * Xe(1,1) + XRCBL * Xe(1,2) + YRCBL * Xe(1,3)
         YEBL1 = CB1 * Xe(2,1) + XRCBL * Xe(2,2) + YRCBL * Xe(2,3)
C
C        CALCUL DES Pchapeau(bl)  POLYNOMES LAGRANGE DE DEGRE 1 + BULLE P3
         BULLE = 9D0 * CB1 * XRCBL * YRCBL
         PRCBL1(1) = CB1   - BULLE
         PRCBL1(2) = XRCBL - BULLE
         PRCBL1(3) = YRCBL - BULLE
         PRCBL1(4) = 3D0 * BULLE
C
...
ccc               WRITE(IMPRIM,*) 'f2snvp1bp1: sortie xydstr: ierr=',ierr
ccc               WRITE(IMPRIM,*) 'f2snvp1bp1: AUCUN TRIANGLE contenant ',
ccc     %                          XEBL0, YEBL0
ccc               WRITE(IMPRIM,*) 'f2snvp1bp1: CAS TRAITE ENSUITE'
ccc            ENDIF
...
C              VITESSE AU NOEUD NONOEU AVANT SORTIE DES TRIANGLES
               IF( IERR .EQ. 4 ) THEN
                  ...
                  ENDIF
10000 FORMAT('f2snvp1bp1: Avant sortie des TRIANGLES: VITESSE CALCULEE='
     %                   ,2G15.6,' VITESSE IMPOSEE =', 2G15.6)
20000 FORMAT('f2snvp1bp1: Before TRIANGLES EXIT: COMPUTED VELOCITY='
...
C              RECHERCHE DU POINT D'INTERSECTION LE PLUS PROCHE DE (XEBL0,YEBL0)
               I0   = 0
               DMIN = 1D100
               DO I=1,3
...
C              LE POINT D'INTERSECTION SUR L'ARETE LA PLUS PROCHE DE XEBL0,YEBL0
               IF( I0 .NE. 0 ) THEN
                  XEBL0 = PTI(1,I0)
                  YEBL0 = PTI(2,I0)
ccc                  WRITE(IMPRIM,*) 'f2snvp1bp1: Point frontiere Calcule',
...
C        Les Pchapeau(X(tn;tn+1,Fe(bl))) POLYNOMES P1 + BULLE
