...
C ----     PAR PROJECTION SUR LES POLYNOMES P1 SUR CHAQUE TRIANGLE
C          Integrale tP1 P1 dX = Integrale tP1 Rot U dX
C          Triangle de BREZZI-FORTIN

C ENTREES:
C --------
C NBNOVI : NOMBRE DE NOEUDS VITESSE SOMMETS et BARYCENTRES DES EF
C XYZNOE : XYZNOE(3,NBNOVI) 3 COORDONNEES DES NOEUDS DU MAILLAGE
C NBELEM : NOMBRE DE TRIANGLES P1B DU MAILLAGE
C NUNOEF : NUNOEF(NBELEM,3) NO DES SOMMETS DES NBELEM EF
...
C     BOUCLE SUR LES TRIANGLES P1B POUR CONSTRUIRE AG
      DO NUELEM = 1, NBELEM
...
C        NUMERO DES 3 SOMMETS DU TRIANGLE NUELEM
         ...
         M = NUNOEF(NUELEM,3)
         NONOEF(3) = M
C        NUMERO DE NOEUD VITESSE DU BARYCENTRE
...
C           NO DU SOMMET M DU TRIANGLE P1B
            NS = NONOEF(M)
