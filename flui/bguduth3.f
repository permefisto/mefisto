      SUBROUTINE BGUDUTH3( CoForc, CoTeNL,
...
C23456---------------------------------------------------------------012
     ...
     %                  NONOSO(NBNOVI), MNELE, NUMIVO, NUMAVO
...
C
      REAL              XYZEF(10,3)
      DOUBLE PRECISION  FORCE(3,10), VJ, VJJ, Coef, SF, SP,
     %                  DELTAe, DF(3,3), DFM1(3,3), DFM1DLa(3,4),
     %                  S, D, X, Y, Z, VN(3), DGL(2,3)
C
...
C
     ...
     %                           2, 3, 4, 6, 10, 9 /
C
C     INTEGRALES P2 DP2 SUR LE TETRAEDRE UNITE de REFERENCE
...
C
C     [DP2(4 Sommets)]  SUR LE TETRAEDRE UNITE
C     DOUBLE PRECISION  DP2S3D(3,10,4)
...
C              COEFFICIENTS DES EFFORTS VOLUMIQUES COMPOSANTE L
               ...
               IF( IEFORC .GT. 0 ) THEN
                  DO J=1,10
                     SF = SF + INTP23D(J) * FORCE(L,J) * CoForc
                  ENDDO
               ENDIF
C
C              TERME NON LINEAIRE DE TRANSPORT COMPOSANTE L
C            - Integrale  tdP1/dxl  CoTeNL  ( (V.Grad) V )l dx
               DO K=1,3
                  DO J=1,10
C
...
C                          COMPOSANTE L DE LA VITESSE AU NOEUD NONOJJ
                           VJJ = VXVYVZ(NONOJJ,L)
C
C                          P2DP23D(i,k,j) = integrale P2j dP2jj/dxn dx dy dz
                           SF = SF - P2DP23D(J,N,JJ) * D * VJJ
C
                        ENDDO
                     ENDDO
                  ENDDO
...
C        Integrale tP1  n . ( - Fg(tn+1)/Rho
C                             + Wm . Grad Wm )    sur Gamma
C        ===========================================================
         DO NF=1,4
C
...
ccc            VN(3) = VN(3) / DELTA
               DO I=1,3
C
C                 COEFFICIENT NOSFTE(I,NF) DE BE
                  S = 0D0
                  DO L=1,3
C
C                    LA FORCE EXTERIEURE SUR LA SURFACE DE CETTE FACE NF
C                    Integrale tP1 n . ( - Fg(tn+1) * CoForc )
                     SF = 0D0
                     IF( IEFORC .GT. 0 ) THEN
                        DO J=1,6
...
C                    LE TERME DE TRANSPORT NON LINEAIRE   Wm . Grad Wm
                     ...
                     DO K=1,3
                        DO J=1,6
C                          COMPOSANTE K DE LA WITESSEm AU NOEUD NONOTE(NSF(J))
                           VJ = VXVYVZ( NONOTE(NSF(J)), K )
                           DO N=1,3
                              D = DFM1(K,N) * VJ
                              DO JJ=1,10
C                                COMPOSANTE L DE LA VITESSE AU NOEUD NONOTE(JJ)
                                 VJJ = VXVYVZ( NONOTE(JJ), L )
                                 DO M=1,3
