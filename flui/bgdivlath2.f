...
!23456---------------------------------------------------------------012
      ...
      INTEGER           NUNOEF(NBELEM,6), NULAEF(3,NBELEM),
     %                  NONOSO(NBNOVI)
     ...
     %                  K1, K2
      DOUBLE PRECISION  DIVUP2(3), TDLDL(3,3)
      DOUBLE PRECISION  DELTA, X21, X31, Y21, Y31,
     %                  DFM1(2,2), DFM1DLA(2,3), VN(2),
...
!        ASSEMBLAGE DE BE(1) DANS BG( NONOSO( NOSOMMET1 ) )
         NS = NONOSO( NS1 )
         BG(NS) = BG(NS)
     %        + ( V1*DIVUP2(1) + V2*DIVUP2(2) + V3*DIVUP2(3) ) * DELTA
