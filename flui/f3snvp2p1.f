      SUBROUTINE F3SNVP2P1( NEF00,   NBEF,   NUNOTE,
...
C     CALCUL DU JACOBIEN DE Fe POLYNOMIALE DE DEGRE 1 de NEF INITIAL
     ...
     %                  Xe(3,2)-Z1, Xe(3,3)-Z1, Xe(3,4)-Z1 )
      ...
      IF( DELTAe1 .LE. 0D0 ) THEN
         IF( LANGAG .EQ. 0 ) THEN
            WRITE(IMPRIM,*) 'f3snvp2p1: ATTENTION EF',NEF00,
     %                 ' de VOLUME*6=',DELTAe1,' Be NON PRIS EN COMPTE'
         ELSE
            WRITE(IMPRIM,*) 'f3snvp2p1: ATTENTION FE',NEF00,
...
C           JACOBIEN de l'EF NEF NUL ou NEGATIF
            IF( LANGAG .EQ. 0 ) THEN
               WRITE(IMPRIM,*) 'f3snvp2p1: attention EF',NEF,
     %         ' de VOLUME*6=',DELTAe,' Be NON CALCULE pour L=',L
            ELSE
               WRITE(IMPRIM,*) 'f3snvp2p1: attention FE',NEF,
...
C              VITESSE AU NOEUD NONOEU AVANT SORTIE DES TETRAEDRES
               PRINT*,'f3snvp2p1: POINT:', XEBL0, YEBL0, ZEBL0,
     %                ' IERR=',IERR
               ...
               IF( IERR .EQ. 4 ) THEN
                  ...
                  ELSE
                     PRINT 20000, V, VA
                  ENDIF
10000 FORMAT(' f3snvp2p1: CAUSE sortie des TETRAEDRES: VITESSE CALCULEE=
     %'            ,3G15.6,' REMPLACEE par VITESSE PRECEDENTE=', 3G15.6)
20000 FORMAT(' f3snvp2p1: Before TETRAHEDRA EXIT: COMPUTED VELOCITY='
...
C                 JACOBIEN de l'EF NEF NUL ou NEGATIF
                  IF( LANGAG .EQ. 0 ) THEN
                     WRITE(IMPRIM,*) 'f3snvp2p1: Probleme EF',NEF,
     %         ' de VOLUME*6=',DELTAe,' Be NON CALCULE pour L=',L
                  ELSE
                     WRITE(IMPRIM,*) 'f3snvp2p1: Problem FE',NEF,
     %         ' of VOLUME*6=',DELTAe,' Be is NOT COMPUTED for L=',L
                  ENDIF
                  GOTO 100
               ENDIF
...
ccc            WRITE(IMPRIM,*)'f3snvp2p1: Point frontiere intersecte',
...
ccc               print*,'f3snvp2p1: XYZBL0=',XYZ0,' PROJETE DANS NEF'
