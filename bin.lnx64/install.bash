...
then
  ...
  echo "see http://www.ann.jussieu.fr/sourceload.php"
else
  LANGAGE=0
  ...
  echo "voir http://www.ann.jussieu.fr/sourceload.php"
fi
...
if test -f mefisto.lnx32.tgz
then
  if [ $LANGAGE = 0 ]
  then
    echo "Installation de la VERSION LINUX 2Giga Processeur 32bits"
  else
    echo "Installation of 2Giga LINUX VERSION Processor 32bits"
  fi
  cp -p mefisto.lnx32.tgz  mefisto.lnx.tgz
  NomMachine=lnx32
  NomBin=lnx32
fi
...
then
  if [ $LANGAGE = 0 ]
  then
    echo "Installation de la VERSION EXECUTABLE LINUX 2Giga Processeur 64bits"
  else
    echo "Installation of 2Giga LINUX EXECUTABLE VERSION Processor 64bits"
  fi
...
then
  ...
  echo "| MM M MM  EEEEE   FFFF     II   SSSSSS    TT   OO   OO |  FAIT L EF !"
  echo "| MM   MM  EE      FF       II       SS    TT   OO   OO |"
  echo "| MM   MM  EEEEEE  FF      IIII  SSSSSS    TT   OOOOOOO | Version April 2012"
  echo "---------------------------------------------------------"
  echo
