      SUBROUTINE TRFETO5( KTITRE,  PTXYZD,  N1FEOC,  NFETOI,
     %                    NBTRCF,  NOTRCF,  LEFACO,
     %                    NBTETR0, NOTETR0, NBTETRA, NOTETRA, NOTETR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    TRACE DES FACES TRIANGULAIRES SIMPLES D'UNE ETOILE
C -----    + TRIANGLES DU CF + TETRAEDRES PRIMITIFS et AJOUTES
...
C NFETOI : 1: NUMERO NOTETR DU TETRAEDRE AU DELA DE LA FACE
C             =0 SI INCONNU
...
C          C-A-D DU POLYGONE DES TRIANGLES PERDUS
C NOTRCF : TABLEAU DU NUMERO DANS LEFACO DES TRIANGLES PERDUS
...
C NBTETR0: NOMBRE DE TETRAEDRES PRIMITIFS DE LA LISTE DES TETRAEDRES A TRACER
C NOTETR0: NUMERO DANS NOTETR DES TETRAEDRES PRIMITIFS A TRACER
C NBTETRA: NOMBRE DE TETRAEDRES DE LA LISTE DES TETRAEDRES A TRACER
C NOTETRA: NUMERO DANS NOTETR DES TETRAEDRES A TRACER
CC NOTETR : LISTE DES TETRAEDRES
...
C
     ...
     %                  NOTETR0(NBTETR0), NOTETRA(NBTETRA), NOTETR(8,*)
...
C     CALCUL DU BARYCENTRE DES TETRAEDRES PRIMITIFS
 4    NBITEM1 = NBITEM
      DO N = 1, NBTETR0
...
C              LE MAXIMUM
 ...
 8    IF( NBITEM .GE. MXITEM ) THEN
         PRINT*,'trfeto5: AUGMENTER MXITEM=',MXITEM
      ENDIF
...
C           TRACE DU TETRAEDRE NOTETR( NT ) AJOUTE
            IF( NT .GT. 0 ) THEN
...
C           TRACE DU TETRAEDRE NOTETR( NT ) PRIMITIF
            IF( NT .GT. 0 ) THEN

C              LE TRACE DES 6 ARETES DU TETRAEDRE NOTETR(*,NT)
               CALL TRTETRA( NCMAGE, NOTETR(1,NT), PTXYZD )
C
C              LE TRACE DU NUMERO DES SOMMETS DES TETRAEDRES DE NOTETRA
               DO I = 1, 4
                  NS = NOTETR(I,NT)
