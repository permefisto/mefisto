...
C     LE TABLEAU LEXIQUE DE CETTE LIGNE
      CALL LXNLOU( NTLIGN, NULIIN, NTLXSI, MN )
      IF( NTLXSI .LE. 0 ) THEN
         NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'LIGNE INITIALE INCONNUE'
         ELSE
            KERR(1) = 'UNKNOWN INITIAL LINE'
         ENDIF
         CALL LEREUR
         IERR = 1
         RETURN
...
C     LE TABLEAU 'NSEF' DE CETTE LIGNE
      ...
      IF( NTNSEFI .LE. 0 ) THEN
         NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'LIGNE SANS TMS NSEF'
         ELSE
            KERR(1) = 'LIGNE WITHOUT TMS NSEF'
         ENDIF
         CALL LEREUR
         IERR = 1
         RETURN
...
C     LE TABLEAU 'XYZSOMMET' DE CETTE LIGNE
      ...
      IF( NTXYZI .LE. 0 ) THEN
         NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'LIGNE SANS TMS XYZSOMMET'
         ELSE
            KERR(1) = 'LIGNE WITHOUT TMS XYZSOMMET'
         ENDIF
         CALL LEREUR
         IERR = 2
         RETURN
...
C     LE NOMBRE DE POINTS A IMPOSER COMME SOMMETS
      NBPOSL = LADEFI( WBPOSL )
      DO K = 1, NBPOSL
C
C        LE NOM DU POINT DEVENANT SOMMET
         NUPOSL = LADEFI( WUPOSL - 1 + K )
C
C        LE TABLEAU LEXIQUE DE CE POINT
         CALL LXNLOU( NTPOIN, NUPOSL, NTPOSL, MNPOSL )
         IF( NTPOSL .LE. 0 ) THEN
            NBLGRC(NRERR) = 1
            IF( LANGAG .EQ. 0 ) THEN
...
C        LE TABLEAU 'XYZSOMMET' DE CE POINT
         CALL LXTSOU( NTPOSL, 'XYZSOMMET', NTXYZP, MNXYZP )
         IF( NTXYZP .LE. 0 ) THEN
            NBLGRC(NRERR) = 1
            IF( LANGAG .EQ. 0 ) THEN
