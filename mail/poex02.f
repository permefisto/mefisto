      SUBROUTINE POEX02( NTLXPO , RADEFI ,
     %                   NTSOPO , MNSOPO , IERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    GENERER LE TABLEAU SOMMET DU POINT ( TYPE DE POINT: 2 )
C -----    A PARTIR DES COORDONNEES CYLINDRIQUES
C ENTREES:
C --------
...
C          RADEFI(WAYOCY) = R    DU POINT
C          RADEFI(WTETAC) = TETA DU POINT
C          RADEFI(WCOTEC) = COTE DU POINT
...
C     LE NOMBRE DE SOMMETS
      MCN( MNSOPO + WNBSOM ) = 1
C
C     PASSAGE DE  R TETA Z  EN  X Y Z
      R    = RADEFI( WAYOCY )
C     CONVERSION EN RADIANS DE LA LONGITUDE
      TETA = RADEFI( WTETAC ) * ATAN( 1D0 ) * 4D0 / 180.D0
      RMCN( MNSOPO + WYZSOM     ) = REAL( R * COS( TETA ) )
      RMCN( MNSOPO + WYZSOM + 1 ) = REAL( R * SIN( TETA ) )
      RMCN( MNSOPO + WYZSOM + 2 ) = RADEFI( WCOTEC )
C
C     AJOUT DE LA DATE
