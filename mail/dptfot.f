      SUBROUTINE DPTFOT( PT,     PTXYZD, NUOT, LARBRO, LARBRT,
     %                   DISMIN, NUFMIN )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    RETROUVER LA PLUS PROCHE FACE DE L'OT NUOT PAR RAPPORT AU
...
C NUFMIN : NUMERO DE L'ARETE DE L'OT DE DISTANCE MIN AU POINT PT
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C        OCTAEDRE
         DO 10 I=1,8
C           RECHERCHE DES 3 SOMMETS DE LA FACE I DE L'OCTAEDRE
            CALL NU3SFO( I, NS1, NS2, NS3 )
            NS1 = LARBRO(14+NS1,NUOT)
            NS2 = LARBRO(14+NS2,NUOT)
            NS3 = LARBRO(14+NS3,NUOT)
C           CALCUL DE LA DISTANCE A LA FACE
            CALL DIPTPL( PT, PTXYZD(1,NS1), PTXYZD(1,NS2),
     %                       PTXYZD(1,NS3), D )
            IF( D .LT. DISMIN ) THEN
C              NOUVEAU MINIMUM
               DISMIN = REAL( D )
...
C        TETRAEDRE
         DO 20 I=1,4
C           RECHERCHE DES 3 SOMMETS DE LA FACE I DU TETRAEDRE
            CALL NU3SFT( I, NS1, NS2, NS3 )
            NS1 = LARBRT(5+NS1,-NUOT)
            NS2 = LARBRT(5+NS2,-NUOT)
            NS3 = LARBRT(5+NS3,-NUOT)
C           CALCUL DE LA DISTANCE A LA FACE
            CALL DIPTPL( PT, PTXYZD(1,NS1), PTXYZD(1,NS2),
     %                       PTXYZD(1,NS3), D )
            IF( D .LT. DISMIN ) THEN
