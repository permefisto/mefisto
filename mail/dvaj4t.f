      SUBROUTINE DVAJ4T( NS,     NBTRNS, NOTRNS, NOARNS,
...
C     TRACE OU NON DES TRIANGLES GENERES DANS LA TRIANGULATION(CF DVTR2D)
     ...
     %                  CETRIA(1:3,1:MXTRIA)
      ...
      DOUBLE PRECISION  SURTD2, S, SMX, XX, YY, XD, YD
C
...
C           SATURATION DES TRIANGLES
            ...
            GOTO 9999
         ENDIF
C        LE NUMERO DU TRIANGLE SUPPLEMENTAIRE
         NDTRIA = MAX( NDTRIA, N1TRVI )
...
C     NUMERO PXYD DU SOMMET DE L'ARETE OPPOSEE A NS DANS NOTRNS(N)
 55   IF( NOARNS(N) .LT. 3 ) THEN
      ...
      XD  = PXYD(1,NPC) - PXYD(1,NPA)
      YD  = PXYD(2,NPC) - PXYD(2,NPA)
C     SINUS DE L'ANGLE ( AN, AC )
      S   = ( XX * YD - YY * XD ) / SQRT( (XD*XD+YD*YD) * (XX*XX+YY*YY))
      IF( S .LE. 0.15D0 ) THEN
...
C        RECTIFICATION POUR LE DERNIER
 ...
 100  CONTINUE
C
C     MISE A JOUR DU CENTRE ET RAYON DU CERCLE CIRCONSCRIT AUX TRIANGLES
      DO 110 I=1,NBTRNS+4
         L  = 1
         NT = NOTRNS(I)
     ...
     %                PXYD(1,NOTRIA(3,NT)),
     %                CETRIA(1,NT), L )
         IF( L .NE. 0 ) WRITE(IMPRIM,*) 'DVAJ4T: TRIANGLE DEG',NT
 110  CONTINUE
C
