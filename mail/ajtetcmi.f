      SUBROUTINE AJTETCMI( KTITRE, PTXYZD,
...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    AJOUT A L'ETOILE DES TETRAEDRES DES ARETES SIMPLES DU CF
C -----    SI UNE ARETE DU CF N'A PAS 2 TETRAEDRES ADJACENTS DANS
C          L'ETOILE ALORS
...
C     --------------------------------------------------------------
C     LISTAGE NOTECF DE TOUS LES TETRAEDRES DES ARETES SIMPLES DU CF
C     --------------------------------------------------------------
      DO NCF=1, NBCF
...
C        LA PREMIERE ARETE DU CF NCF
         NA0 = N1ARCF(NCF)
         IF( NA0 .LE. 0 ) THEN
            print*,'ajtetcmi: ERREUR N1ARCF=',NA0,' VALEUR INCORRECTE'
            GOTO 9999
         ENDIF
         NA1 = NA0
...
C        AJOUT DE TOUS LES TETRAEDRES D'ARETE NSA1 NSA2 A NOTECF
     ...
     %                NBTEA, MXTECF-NBTECF, NOTECF(NBTECF+1), IER )
         NBTECF = NBTECF + NBTEA

         IF( NBTEA .LE. 0 ) THEN
            PRINT*,'ajtetcmi: ANORMALE ARETE NOARCF',NA1,' du CF:',
     %              NSA1,NSA2,' DANS AUCUN TETRAEDRE du MAILLAGE'
            IERR = IERR + IER
C           TRACE DES FACES TRIANGULAIRES DU CF
            tracte = .true.
            KTITRE='ajtetcmi: ANORMALE ARETE                 dans AUCUN 
...
C        UNICITE DES NO DE TETRAEDRES DU TABLEAU NOTECF
         ...
         IF( NA2 .NE. NA0 .AND. NA2 .NE. 0 ) THEN
C           PASSAGE A L'ARETE SUIVANTE DU CF
      ...
      IF( IERR .GT. 0 ) THEN
C        AU MOINS UNE ERREUR RENCONTREE: PAS DE TETRAEDRE DANS L'ETOILE
         NBTECF = 0
         GOTO 9999
      ENDIF

C     ----------------------------------------------------------------------------
C     RETRAIT DU TETRAEDRE DE FACE DE COSINUS MINIMAL DE TOUTES LES ARETES SIMPLES
C     ----------------------------------------------------------------------------
      DO NCF=1, NBCF
...
C                 RECHERCHE DU 3-EME SOMMET DU TRIANGLE NTR DU CF
                  IF( N2 .EQ. 3 ) THEN
                     N3 = 1
                  ELSE
                     N3 = N2+1
...
C                 TRACE pour VOIR le RETRAIT de TETRAEDRES
         KTITRE='ajtetcmi : RETRAIT du TETRAEDRE          d''ARETE       
     %             de COS MINIMAL'
                  WRITE(KTITRE(33:40),'(I8)') NTCMIN
                  WRITE(KTITRE(51:58),'(I8)') NSA1
                  WRITE(KTITRE(60:67),'(I8)') NSA2
                  CALL SANSDBL( KTITRE, L )
                  PRINT*, KTITRE(1:L)
                  CALL TRCFFAPE( KTITRE(1:L), PTXYZD,
     %                           MXFACO, LEFACO, NO0FAR, NOTETR,
...
C                    PAS DE RETRAIT DE TETRAEDRE DANS NOTECF
                     PRINT*,'ajtetcmi: PROBLEME PAS de RETRAIT DE TETRAE
 ...
 50      IF( NA2 .NE. NA0 .AND. NA2 .NE. 0 ) THEN
C           PASSAGE A L'ARETE SUIVANTE DU CF
            NA1 = NA2
            GOTO 20
         ENDIF
...
C     TRACE DES TETRAEDRES, TRIANGLES, ARETES DU CF et POINTS D'INTERSECTION
      IF( NBTECF .GT. 0 .OR. IERR .GT. 0 ) THEN
        KTITRE='ajtetcmi: les          TETRAEDRES de l''ETOILE IERR=   '
         WRITE(KTITRE(15:19),'(I5)') NBTECF
         WRITE(KTITRE(53:55),'(I3)') IERR
         CALL SANSDBL( KTITRE, L )
         PRINT*, KTITRE(1:L)
