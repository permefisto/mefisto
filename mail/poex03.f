      SUBROUTINE POEX03( NTLXPO , RADEFI ,
     %                   NTSOPO , MNSOPO , IERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    GENERER LE TABLEAU SOMMET DU POINT ( TYPE DE POINT: 2 )
C -----    A PARTIR DES COORDONNEES SPHERIQUES
C ENTREES:
C --------
...
C          RADEFI(WAYONS) = R    DU POINT
C          RADEFI(WTETAS) = TETA DU POINT
C          RADEFI(WCPHIC) = PHI  DU POINT
...
C
      REAL              RADEFI(0:*)
      DOUBLE PRECISION  PI,R,TETA,PHI
C
...
C     LE NOMBRE DE SOMMETS
      MCN( MNSOPO + WNBSOM ) = 1
C
C     PASSAGE DE  R TETA PHI  EN  X Y Z
      R    = RADEFI( WAYONS )
C     CONVERSION EN RADIANS DE LA LONGITUDE
      PI   = ATAN( 1D0 ) * 4D0 / 180.D0
      TETA = RADEFI( WTETAS ) * PI
C     CONVERSION EN RADIANS DE LA LATITUDE
      PHI  = RADEFI( WPHIS ) * PI
      RMCN( MNSOPO + WYZSOM     ) = REAL( R * COS( PHI ) * COS( TETA ) )
      RMCN( MNSOPO + WYZSOM + 1 ) = REAL( R * COS( PHI ) * SIN( TETA ) )
      RMCN( MNSOPO + WYZSOM + 2 ) = REAL( R * SIN( PHI ) )
C
C     AJOUT DE LA DATE
