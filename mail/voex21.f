      SUBROUTINE VOEX21( NTLXVO , LADEFI , RADEFI ,
     &                   NTCUVO , MNCUVO , NTSOCU , MNSOCU , IERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :     GENERER LE MAILLAGE D'UN CYLINDRE
C ----
C
C ENTREES :
C --------
...
C           CF '~TD/DAT/A_VOLUMES__DEFINITION'
C
C SORTIES :
C ---------
...
C     ===========================
     ...
     %              // KERR(MXLGER)(1:15)
      ...
      ENDIF
C
C     RECUPERATION DES 3 COORDONNEES DU SOMMET DU CYLINDRE
C     ====================================================
...
C
      IF ( Z.EQ.0 ) THEN
         NBLGRC(NRERR) = 1
         KERR(1) =  'SOMMET DU CYLINDRE INCORRECT'
         CALL LEREUR
         IERR = 1
         RETURN
      ENDIF
...
C     CONSTRUCTION DU TABLEAU 'NSEF' DE CE VOLUME
      NBCUVO = NBSEC*(NBR-1)*(NBH-1)
      CALL LXTNDC(NTLXVO,'NSEF','ENTIER',WUSOEF+8*NBCUVO)
      CALL LXTSOU(NTLXVO,'NSEF',NTCUVO,MNCUVO)
C
C     GENERATION DES SOMMETS ET ELEMENTS DU CYLINDRE
C     ==============================================
      CALL CYLINN( NBSEC , NBR , NBH , X , Y , Z , RAYOCI ,
...
C
      END
C234567--------------------------------------------------------------012
      SUBROUTINE CYLINN( NBSEC , NBR , NBH , X , Y , Z , RI , RS ,
     &                   QH , QR ,QS ,
     &                   XYZSOM , NOEUDS )
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT    : GENERER LE MAILLAGE D'UN CYLINDRE
...
C X      : COORDONNEE X DU SOMMET DU CENTRE DU CERCLE SUPERIEUR
C Y      : COORDONNEE Y DU SOMMET DU CENTRE DU CERCLE SUPERIEUR
C Z      : COORDONNEE Z DU SOMMET DU CENTRE DU CERCLE SUPERIEUR
...
C
      ...
      DO K=1,NBH
         DO J=2,NBR
            DO I=1,NBSEC
               ...
               VJ = DIVISN(QR,J,NBR)
               WI = DIVISN(QS,I,L)
               XYZSOM(1,NUMS)=DIVISN(QH,K,NBH)*X+RK*VJ*COS(2*PI*WI)
               XYZSOM(2,NUMS)=DIVISN(QH,K,NBH)*Y+RK*VJ*SIN(2*PI*WI)
               XYZSOM(3,NUMS)=DIVISN(QH,K,NBH)*Z
            ENDDO
         ENDDO
      ENDDO
      ...
      DO K=1,NBH-1
         DO I=1,NBSEC
            ...
            NOEUDS(2,NUMEF) = (K-1)*NBSN+1+I
            NOEUDS(3,NUMEF) = (K-1)*NBSN+2+MOD(I,NBSEC)
            NOEUDS(4,NUMEF) = K*NBSN+1
            NOEUDS(5,NUMEF) = K*NBSN+1+I
            NOEUDS(6,NUMEF) = K*NBSN+2+MOD(I,NBSEC)
            NOEUDS(7,NUMEF) = 0
            NOEUDS(8,NUMEF) = 0
            DO J=2,NBR-1
               NUMEF = (K-1)*NBELFN+(J-1)*NBSEC+I
               NOEUDS(1,NUMEF) = (K-1)*NBSN+2+(I-1)+(J-2)*NBSEC
               NOEUDS(2,NUMEF) = (K-1)*NBSN+2+(I-1)+(J-2)*NBSEC+NBSEC
             NOEUDS(3,NUMEF)=(K-1)*NBSN+2+MOD(I,NBSEC)+(J-2)*NBSEC+NBSEC
               NOEUDS(4,NUMEF) = (K-1)*NBSN+2+MOD(I,NBSEC)+(J-2)*NBSEC
               NOEUDS(5,NUMEF) =  K*NBSN+2+(I-1)+(J-2)*NBSEC
               NOEUDS(6,NUMEF) =  K*NBSN+2+(I-1)+(J-2)*NBSEC+NBSEC
               NOEUDS(7,NUMEF) = K*NBSN+2+MOD(I,NBSEC)+(J-2)*NBSEC+NBSEC
               NOEUDS(8,NUMEF) =  K*NBSN+2+MOD(I,NBSEC)+(J-2)*NBSEC
            ENDDO
         ENDDO
      ENDDO
