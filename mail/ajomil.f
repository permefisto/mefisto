      SUBROUTINE AJOMIL( KNMVOL, COSMAX, MXFACO, LEFACO, NBFACO, N1FASC,
...
C     SOIT  UN ANGLE ARCOS(-0.85)=148.21 DEGRES
     ...
     %                  NOSTIS(MXSOMM)
      ...
      tracte0 = tracte
      NBSOM0  = NBSOMM
      PRINT*,'ajomil: Volume: ',KNMVOL
      PRINT*,'ajomil: Debut avec COSMIN=',COSMIN,' COSMAX=',COSMAX,
     %       ' NBFAPE=',NBFAPE,' FACES PERDUES  NBSOM0=',NBSOMM
      tracte = .true.

C     TRACE DES FACES LEFACO AVANT AJOUT DE POINTS MILIEUX
      LORBITE = 1
...
C           ---------------------
C           UNE FACE TRIANGULAIRE
C           ---------------------
C           LAQUELLE DES 3 ARETES DE NF EST LA PLUS LONGUE?
            DMAX = -1D0
            DO I=1,3
               IF( I .LT. 3 ) THEN
                  I1 = I + 1
               ELSE
                  I1 = 1
               ENDIF
               D = DIS2ST( PTXYZD( 1, LEFACO( I +1, NF ) ),
     %                     PTXYZD( 1, LEFACO( I1+1, NF ) ) )
               IF( D .GT. DMAX ) THEN
                  DMAX = D
                  NAR  = I
               ENDIF
...
C           AJOUT DU POINT MILIEU DE LA PLUS GRANDE DES 3 ARETES DE NF
C           ----------------------------------------------------------
...
C              -------------------------------------------------------------
 170           CALL VD1F2F( 1, NAR, NF, MXFACO, LEFACO, NBFACO,
     %                      N1FASC, NBFAPE, NOFAPE,
     %                      NBSOMM, MXSOMM, PTXYZD, NPSOFR,
     %                      MXPIFA, MNPIFA, IERR )
               IF( IERR .NE. 0 ) GOTO 9900
...
C                 SATURATION DES ARETES DE LARETE
                  NBLGRC(NRERR) = 1
                  KERR(1) ='ajomil: TABLEAU LARETE SATURE a AUGMENTER'
                  CALL LEREUR
                  IERR = 3
                  GOTO 9900
               ENDIF
...
C     DESTRUCTION DE LA QUEUE
 9900 CALL TNMCDS( 'ENTIER', MXPIFA, MNPIFA )
      ...
      PRINT*,'ajomil: Fin avec NBSOMM=',NBSOMM,' soit',
     %        NBSOMM-NBSOM0,' POINTS MILIEUX AJOUTES'
