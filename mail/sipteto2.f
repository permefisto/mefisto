      SUBROUTINE SIPTETO2( NOPASS6,   KTITRE,  PTXYZD, NPSOFR,
     %                     N1FEOC,    NFETOI,  NBFETO, MAXFAC, XYZMAX,
...
C ENTREES:
C --------
C NOPASS6: =0 => RETOUR APRES LE TEST DE LA GRILLE AUTOUR DU BARYCENTRE
...
ccc      include"./incl/darete.inc"
     ...
     %                  AREMOY, VTETREG, C, V, VETXYZ, RAVEVM,
...
ccc 1    KTITRE='Entree sipteto2:        FACES de NFETOI NoPass6=     '
ccc      WRITE(KTITRE(17:21),'(I5)') NBFETO
ccc      WRITE(KTITRE(48:48),'(I1)') NOPASS6
...
C        COEFFICIENT REDUCTEUR POUR UNE ETOILE CORRECTEMENT VOLUMINEUSE
      ...
      IF( NOPASS6 .EQ. 1 ) THEN

C        1-ER APPEL DE SIPTETO2: GRILLE AUTOUR DU BARYCENTRE
C        ===================================================
C        LES PAS ENTRE SOMMETS DE LA GRILLE
         NBSUB = 3
         DO N=1,3
            XYZN(N) = (XYZMIX(N,2)-XYZMIX(N,1)) / (2.1 * NBSUB) * COEFQP
         ENDDO
...
C           =============================================
            MIARSIMI = -1
            NOCHOIXYZ = 1
       KTITRE='sipteto2:  TOUTES LES        FACES+XYZMAX -> TETRAEDRES V
     %>0 NOCHOIXYZ=   '
       ...
       KTITRE='sipteto2: PARMI les       FACES seules       FACES+XYZMAX
...
C        TABLEAU QUI A PEUT ETRE ETE ECRASE PAR LES POINTS SUIVANTS XYZMAX
     ...
     %                  NBFETO, NBVPIN, QUAMIMX )
         ...
         IF( NBVPSI .GE. MAXFAC ) THEN
C           ON SE CONTENTE DES NBVPSI FACES + XYZMAX
            GOTO 9900
         ENDIF
...
C     ------------------------------------------------------------
      NBVPSI = 0
      KTITRE='sipteto2 ap2:        FACES+XYZ TETRAEDRES V>0       FACES 
     %NOCHOIX=  QUAMIN=              VETXYZ=                        '
...
C              RECHERCHE DU POINT DONNANT LE MINIMUM D'ARETES SIMPLES DES CIRCUITS
     ...
     %           ( MIARSICI.EQ.MIARSIMI .AND. QUAMIN.GT.QUAMIMX) )) THEN
      ...
      KTITRE='sipteto2 ap3:        FACES+XYZ TETRAEDRES V>0 pour        
     %FACES NOCHOIXYZ=   '
      WRITE(KTITRE(14:18), '(I5)'   ) NBVPSI
...
ccc                           QUAMIMX = QUAMIN
ccc                           print *,'Sipteto2: nochoixyz=',nochoixyz,
...
C        PASSAGE A LA FACE SUIVANTE DE L'ETOILE
 40      NF = NFETOI(5,NF)
      ...
      IF( NBVPSIMAX .GT. 0 ) THEN
      KTITRE='sipteto2 ap4:        FACES+XYZ TETRAEDRES V>0 pour        
     %FACES NOCHOIXYZ=   QMIN=          VETXYZ=            '
      WRITE(KTITRE(14:18), '(I5)'   ) NBVPSI
      WRITE(KTITRE(51:55), '(I5)'   ) NBFETO
      WRITE(KTITRE(74:75), '(I2)'   ) NOCHOIXYZ
...
C        PASSAGE A L'ARETE SUIVANTE DE NF
 80      NF = NFETOI(5,NF)
      ...
      IF( NBVPSIMAX .GT. 0 ) THEN
      KTITRE='sipteto2 ap5:        FACES+XYZ TETRAEDRES V>0 pour        
     %FACES NOCHOIXYZ=   QMIN=          VETXYZ=            '
      WRITE(KTITRE(14:18), '(I5)'   ) NBVPSI
      WRITE(KTITRE(51:55), '(I5)'   ) NBFETO
      WRITE(KTITRE(74:75), '(I2)'   ) NOCHOIXYZ
...
C        PASSAGE A LA FACE SUIVANTE
 130     NF = NFETOI(5,NF)
      ...
      IF( NBVPSIMAX .GT. 0 ) THEN
      KTITRE='sipteto2 ap6:        FACES+XYZ TETRAEDRES V>0 pour        
     %FACES NOCHOIXYZ=   QMIN=          VETXYZ=            '
      WRITE(KTITRE(14:18), '(I5)'   ) NBVPSI
      WRITE(KTITRE(51:55), '(I5)'   ) NBFETO
      WRITE(KTITRE(74:75), '(I2)'   ) NOCHOIXYZ
...
ccc         print *,'fin sipteto2: avant f1ptvpsi nochoixyz=',nochoixyz,
ccc     %           ' NBVPSIMAX=',NBVPSIMAX,' NFVPSIMAX=',NFVPSIMAX,
...
C           NFVPSI DE NFETOI
     ...
     %                     MXASFVP, MIARSICI, NBASFVP, NSASFVP, IERR )
            ...
            PRINT*,'sipteto2: QUAMIMX=',QUAMIMX,' AU BARYCENTRE DES ',
...
C     ESSAI DE MODIFIER LES XYZ D'UN SOMMET NON CF DE L'ETOILE
      PRINT*,'sipteto2 : ESSAI DE MODIFIER XYZ D''UN SOMMET NON CF DE L'
...
C              VN=S1S2xS1S3 EST LA NORMALE DIRIGEE VERS L'INTERIEUR DE L'ETOILE
               ...
               PRINT*,'sipteto2 : MODIF de PTXYZD(',NST,')=',
...
C     ----------------------------------------------------------------------
 8900 NOCHOIXYZ = -1
      ...
      XYZMAX(2) = 1D10
      XYZMAX(3) = 1D10
      PRINT*,'sipteto2 : AUCUN TETRAEDRE XYZMAX+FACE SANS INTERSECTION a
     %vec les',NBFETO,' FACES N''A UN VOLUME>0'


C     ===========================
C     BILAN de SORTIE de sipteto2
C     ===========================
ccc 9000 PRINT *,'sipteto2: NOPASS6=',NOPASS6,' NOCHOIXYZ=',NOCHOIXYZ,
ccc     %        ' XYZ=',XYZ,
ccc     %        ' =>',NBVPSI,' TETRAEDRES V>0 de QUALITE MIN=',QUAMIMX,
ccc     %        ' pour',NBFETO,' FACES de l''ETOILE'
ccc      PRINT *,'sipteto2: les',NBVPSI,' FACES NFVPSI=',
ccc     %        (NFVPSI(K),K=1,NBVPSI)
ccc      print *,'Fin sipteto2: Xmin=',XYZMIX(1,1),' Pt X=',XYZMAX(1),
ccc     %                    ' XMax=',XYZMIX(1,2)
ccc      print *,'Fin sipteto2: Ymin=',XYZMIX(2,1),' Pt Y=',XYZMAX(2),
ccc     %                    ' YMax=',XYZMIX(2,2)
ccc      print *,'Fin sipteto2: Zmin=',XYZMIX(3,1),' Pt Z=',XYZMAX(3),
ccc     %                    ' ZMax=',XYZMIX(3,2)
...
C     ET MAXIMISANT LE MINIMUM DES QUALITES DES TETRAEDRES FORMES
 9100 IF( MIARSIMI .NE. 2**30 ) THEN
      KTITRE='sipteto2 : MinArSiMi=          FACES+XYZMAX TETRAEDRES V>0
     % pour        FACES NOCHOIXYZ=   QMIN=          VETXYZ=           '
         WRITE(KTITRE(21:23), '(I3)' ) MIARSIMI
         WRITE(KTITRE(25:29), '(I5)' ) NBVPSI
      ELSE
      KTITRE='sipteto2 :            PAS  DE  FACES+XYZMAX TETRAEDRES V>0
     % pour        FACES NOCHOIXYZ=   QMIN=          VETXYZ=           '
      ENDIF
...
C     AJOUT AU TITRE DU TRACE le NoPass6
 9900 CALL SANSDBL( KTITRE, L )
      KTITRE = KTITRE(1:L) // ' NoPass6=   '
      WRITE(KTITRE(L+10:L+11), '(I2)' ) NOPASS6
      CALL SANSDBL( KTITRE, L )
      PRINT*, KTITRE(1:L)
