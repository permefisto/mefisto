       SUBROUTINE ITEML2( XY , NOMOBJ , NUOBJT )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C ENTREES :
C ---------
C XY     : COORDONNEES 2D OBJET DE L'ITEM-LIGNE
...
C        TRACE DE L'ITEM SELON LE SYMBOLE '#'
         IF( LPLIGN .NE. 0 ) THEN
            CARSYM = '#' // NOMOBJ
C           LE NOMBRE DE CARACTERES DE CARSYM
            NB = NUDCNB( NOMOBJ ) + 1
C           TRACE DE LA CHAINE = SYMBOLE + EVENTUEL NOM
            CALL SYMBOLE2D( NCOLIG, XY(1), XY(2), CARSYM(1:NB) )
         ENDIF
      ENDIF
      END
