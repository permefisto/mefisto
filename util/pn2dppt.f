      SUBROUTINE PN2DPPT( M, NBPM, PM, NBPOINT, XYPOINT, DPS )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C NBPOINT: NOMBRE DE POINTS DE R2
...
C23456---------------------------------------------------------------012
      DOUBLE PRECISION  PM( 0:M, 0:M, NBPM ), XYPOINT( 2, NBPOINT ),
     %                  DPS( 2, NBPM, NBPOINT )
...
C
C              DPj/Dxl ( XYPOINT(K) )
               CALL PN2DVA( M+1, DPj, XYPOINT(1,K), XYPOINT(2,K),
     %                      DPS(L,J,K) )
C
            ENDDO
C
...
10000 FORMAT('VALEUR de DP',I1,' aux ',I2,' POINTS de R2' )
      DO K=1,NBPOINT
         DO J = 1, NBPM
            DO L = 1, 2
               IF(ABS(DPS(L,J,K)).LT.1D-14) DPS(L,J,K)=0D0
            ENDDO
            PRINT 10001,(L,J,K,DPS(L,J,K),L=1,2)
         ENDDO
      ENDDO
10001 FORMAT(2('DPS(',I1,',',I2,',',I1,')=',D25.17,'   '))
C
      PRINT 10002,((( DPS(L,J,K),L=1,2),J=1,NBPM),K=1,NBPOINT)
