      SUBROUTINE TRITRDMAJ( NB, A, NOANC, PER, VAL )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    MISE A JOUR POUR UNE NOUVELLE VALEUR D'UN REEL DANS LE
C -----    TRI CROISSANT DU TABLEAU A DE NB REELS PAR LA METHODE DU TAS
...
C PER    : NUMERO NOUVEAU DE LA VALEUR DE A() A METTRE A JOUR DANS LE TRI
C
C SORTIES:
C --------
...
C ...................................................................012
      INTEGER           NOANC(1:NB)
      INTEGER           PER, FIL, FILS1, FILS2, FIN, GPER
      DOUBLE PRECISION  A(1:NB), AUX, VAL
C
...
C
 10   GPER = PER/2
      IF( GPER .GT. 0 ) THEN
C
C        IL EXISTE UN GRAND-PERE
         IF( A(PER) .GT. A(GPER) ) THEN
C
C           IL FAUT REMONTER LA VALEUR
C           LA PERMUTATION PERE-GRANDPERE
            AUX     = A(PER)
            A(PER)  = A(GPER)
            A(GPER) = AUX
C           LE POINTEUR EST AUSSI PERMUTE
            NAUX        = NOANC(PER)
            NOANC(PER)  = NOANC(GPER)
            NOANC(GPER) = NAUX
C           LE NOUVEAU PERE EST SON GRANDPERE
            PER = GPER
            GOTO 10
C
         ELSE
C
C           UN DE SES FILS EST IL PLUS GRAND QUE LUI?
C           LE FILS 1 DU PERE
 30         FILS1 = 2 * PER
            IF( FILS1 .LT. NB ) THEN
