      SUBROUTINE GAUSDE( N, M, A, DET, IER )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : RESOUDRE UN SYSTEME LINEAIRE A * X  =  B DE M SECONDS MEMBRES
C ----- PAR LA METHODE DE GAUSS A PIVOT TOTAL
C       ET CALCUL DU DETERMINANT DE LA MATRICE A
C       MODIFICATION DU fichier $MEFISTO/util/gauspt.f
C
C ENTREES :
C ---------
...
C SORTIES:
C --------
C DET : DETERMINANT DE LA MATRICE A
...
CCC      COMMON / UNITES / LECTEU,IMPRIM,NUNITE(30)
      DOUBLE PRECISION  A(N,*), DET, P, S
      INTEGER SIGNE
C
C     LE SIGNE DU DETERMINANT CHANGE A CHAQUE PERMUTATION DE COLONNE
      SIGNE = 1
      DET   = 1D0
C
...
100   CONTINUE
C
C     LE DETERMINANT DE LA MATRICE
      DET = DET * SIGNE
CCC      WRITE(IMPRIM,10100) N, N, DET
CCC10100 FORMAT('GAUSDE: DETERMINANT DE LA MATRICE ',I1,'x',I1,' =',D23.16)
C
