      SUBROUTINE LIDFLA( NLD , NCD , NLF , NCF , NRETOU )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C ENTREES :
C ---------
C NLD NCD : NUMERO DE LIGNE ET COLONNE DANS KLG DE LA CHAINE DEFLAB
C
C SORTIES :
C ---------
...
C23456---------------------------------------------------------------012
      ...
      CHARACTER*1        CAR
C
C     LE DERNIER CARACTERE DE DEFLAB
      NL = NLD
      NC = NCD + 5
C
...
C        LE 1-ER CARACTERE N'EST PAS  UNE LETTRE LICITE
         NBLGRC(NRERR) = 1
         KERR(1) = 'LU: INCORRECT NAME of LABEL '//KLG(NL)(NC:NBCALI)
         CALL LEREUR
         GOTO 9000
      ENDIF
C
...
C        LE CARACTERE EST ILLICITE
         NBLGRC(NRERR) = 1
         KERR(1) = 'LU: INCORRECT LABEL NAME ' //
     %                  KLG(NL)(NC:NBCALI)
         CALL LEREUR
         GOTO 9000
      ENDIF
...
C     TRAITEMENT DE FIN DE NOM DE ETIQUETTE KLG(NL)(NCID:NCF)
 300  IF( NBETIQ .GE. MXETIQ ) THEN
         NBLGRC(NRERR) = 1
         KERR(1) =  'LU: The STACK of LABELS is SATURATED'
         CALL LEREUR
         GOTO 9000
      ENDIF
C
...
C     ERREUR DETECTEE
 9000 NBLGRC(NRERR) = 2
      KERR(1) =
     %'LU: DEFLAB LABEL_NAME{,LABEL_NAME} ; is INCORRECT'
      KERR(2) =  KLG(NLD)
      CALL LEREUR
      NRETOU = 1
      END
