      SUBROUTINE XXTSTD( NOTYOB , NUOBJT , NBPTLX , MNPTLX )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C ENTREES :
C ---------
C NOTYOB  : NUMERO DU TYPE DE L'OBJET
C          (1:POINT 2:LIGNE ... 5:OBJET 6:FONCTIONS)
...
C           LE NUMERO DU TYPE DE L'OBJET  CF FUNCTION NTYOBJ  VAUT
C           1: POINT 2:LIGNE 3:SURFACE 4:VOLUME 5:OBJET 6:TRANSFORMATION
C ATTENTION:LE TABLEAU EST DECLARE ET EST OUVERT
C           SA DESTRUCTION SE FERA A L'EXTERIEUR PAR
...
C
      IF( NOTYOB .EQ. 6 ) THEN
C        PAS DE POINTEUR DANS LES LEXIQUES POUR UNE FONCTION
         RETURN
      ENDIF
C
...
C     LE NOM DU TYPE DE L'OBJET
      KNOM = NMTYOB( NOTYOB )
      I    = INDEX( KNOM , ' ' ) - 1
C     LE NOM DE L'OBJET DANS SON LEXIQUE PERE
      CALL NMOBNU( KNOM(1:I) , NUOBJT , KNOM1 )
      L    = INDEX( KNOM1 , ' ' ) - 1
C     LE NOM DU TABLEAU DEFINITION DE L'OBJET
      KNOMTS = '~>' // KNOM(1:I) // '>' //  KNOM1(1:L) //
     %         '>' // 'DEFINITION'
C
...
C     C'EST L'ENTIER APRES LE DOUBLE PRECISION DATE
     ...
     %    NOTADS(LHTMS) .GT. NBTD ) THEN
          ...
          KERR(1) =
     %  ' XXTSTD: NUMERO INCORRECT DE TABLEAU DESCRIPTEUR'//
     %    KERR(MXLGER)(1:12)
          CALL LEREUR
          RETURN
      ENDIF
...
C           NOM DU TD ET DU TS
     ...
     %                 , NB )
            IF( NB .EQ. 0 ) THEN
               NBLGRC(NRERR) = 3
               KERR(1) =  ' XXTSTD: FUSION IMPOSSIBLE DES NOMS'
...
C              OUVERTURE
               ...
               IF( MNTAMS(LHTMS) .LE. 0 ) THEN
                  NBLGRC(NRERR) = 2
                  KERR(1) = 'XXTSTD: TMS NON OUVRABLE '
                  KERR(2) = NOMTS(LHTMS)
                  CALL LEREUR
                  RETURN
               ENDIF
...
C
 110           LHKTD = LHKTD + 1
               IF( LHKTD .GT. MXLITD ) THEN
                  NBLGRC(NRERR) = 1
                  KERR(1) = 'TABLEAU KTD SATURE. AUGMENTER MXLITD'
                  CALL LEREUR
                  GOTO 9000
               ENDIF
C
