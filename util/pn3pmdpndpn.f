      SUBROUTINE PN3PMDPNDPN( M, NBPM, PM,  N, NBPN, PN,  PMDPNDPN )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    CALCUL DES INTEGRALES PM DPN DPN dx dy dz SUR LE TETRAEDRE UNITE
C -----
C
C ENTREES:
C --------
...
C SORTIE :
C --------
C PMDPNDPN : TABLEAU DES INTEGRALES PMi dPNj/dxk dPNjj/dxl dx dy dz
...
C23456---------------------------------------------------------------012
     ...
     %                  PN( 0:N, 0:N, 0:N, NBPN ),
     %                  PMDPNDPN( NBPM, 3,NBPN, 3,NBPN )
      DOUBLE PRECISION  DP(300), P(1000), DQ(300), Q(1000)
C
      DO J = 1, NBPN
         DO K = 1, 3
...
C                    DERIVATION DQ = dPNjj/dxl
                     CALL PN3DDE( L, N+1, PN(0,0,0,JJ), DQ )
C
C                    PRODUIT PMi dPNj/dxk dPNjj/dxl
                     CALL PN3DPR( 0,M+N+1,P, 0,N+1,DQ,  M+N+1+N,Q )
C
C                    INTEGRALE DE PMi dPNj/dxk SUR LE TETRAEDRE UNITE
                     CALL PN3DIN( 0,M+N+1+N,Q, PMDPNDPN(I,K,J,L,JJ) )
                  ENDDO
               ENDDO
C
            ENDDO
...
C
      PRINT *
      PRINT 10000,M,N,N
10000 FORMAT('INTEGRALE sur TETRAEDRE UNITE de P',I1,' DP',I1,' DP',I1,
     %' dx dy dz')
C
      DO JJ = 1, NBPN
         DO L=1,3
     ...
     %                       PMDPNDPN(I,K,J,L,JJ)=0D0
         ...
         ENDDO
      ENDDO
10001 FORMAT(4('PNDPMDPM(',I2,',',I1,',',I2,',',I1,',',I2,')=',D25.17,
     %'   '))
C
      PRINT 10002,(((((PMDPNDPN(I,K,J,L,JJ),I=1,NBPM),K=1,3),J=1,NBPN),
     %                                                L=1,3),JJ=1,NBPN)
