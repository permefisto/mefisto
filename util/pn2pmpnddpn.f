      SUBROUTINE PN2PMPNDDPN( M, NBPM, PM,  N, NBPN, PN, PMPNDDPN )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C23456---------------------------------------------------------------012
     ...
     %                  PMPNDDPN( NBPM, NBPN, 2,2, NBPN )
      DOUBLE PRECISION  DP(200), P(1000), DDP(200), Q(1000)
C
      DO J = 1, NBPN
C
         DO I = 1, NBPM
C           PRODUIT PMi PNj
            CALL PN2DPR( 0,M+1,PM(0,0,I), 0,N+1,PN(0,0,J), M+N+1,P )
C
            DO JJ = 1, NBPN
               DO L=1,2
C                 DERIVATION DP = dPNjj/dxl
                  CALL PN2DDE( L, N+1, PN(0,0,JJ), DP )
C
                  DO K=1,2
C                    DERIVATION DDP = ddPNjj/dxkdxl
                     CALL PN2DDE( K, N+1, DP, DDP )
C
C                    PRODUIT PMi PNj ddPNjj/dxldxk
                     CALL PN2DPR( 0,M+N+1,P, 0,N+1,DDP,  M+N+1+N,Q )
C
C                    INTEGRALE DE PMi PNj ddPNjj/dxldxk SUR LE TRIANGLE UNITE
                     CALL PN2DIN( 0,M+N+1+N,Q, PMPNDDPN(I,J,K,L,JJ) )
                  ENDDO
               ENDDO
C
            ENDDO
