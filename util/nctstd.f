      SUBROUTINE NCTSTD( LIRECR , KNOMTS , NFFICA , NRETOU )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : CONVERTIR UN TABLEAU TMS NUMERIQUE EN UN FICHIER DE CARACTERES
C ----- A L'AIDE DU TABLEAU DESCRIPTEUR (LIRECR=0)
C       OU LE CONTRAIRE (LIRECR=1)
...
C ENTREE :
C --------
C LIRECR : 0 SI NUMERIQUE => CARACTERES , 1 SI CARACTERES => NUMERIQUE
...
C
      IF( LIRECR .EQ. 0 ) THEN
C
C        CONVERSION NUMERIQUE => CARACTERES
C        ----------------------------------
C        OUVERTURE DU TABLEAU MS
         ...
         IF( NOTAMS(LHTMS) .LE. 0 ) THEN
            ...
            RETURN
         ENDIF
C        LES 2 ADRESSES MCN SONT EGALES
         MCTAMS(LHTMS) = MNTAMS(LHTMS)
         NBLGRC(NRERR) = 1
         KERR(1) =  ' MISE SUR FICHIER DU TABLEAU: '//KNOMTS
         CALL LERESU
C        LE NOM DU TMS EST MIS DANS LE FICHIER FICA OU PLUS EXACTEMENT
C        DANS LE BUFFER DU FICHIER FICA
         CALL FICAKC( NFFICA , KNOMTS )
C        LE NUMERO DANS DICOTD DU NOM DU TABLEAU DESCRIPTEUR
C        C'EST LE 3-EME MOT DE TOUT TABLEAU TMS
         NOTADS(LHTMS) = MCN( MNTAMS(LHTMS) + 2 )
         IF( NOTADS(LHTMS) .LE. 0 .OR.
     %       NOTADS(LHTMS) .GT. NBTD ) THEN
             NBLGRC(NRERR) = 1
             WRITE(KERR(MXLGER)(1:12),'(I12)') NOTADS(LHTMS)
             KERR(1) =
     %     ' NCTSTD: NUMERO INCORRECT DE TABLEAU DESCRIPTEUR='
     %            //KERR(MXLGER)(1:12)
             CALL LEREUR
             RETURN
         ENDIF
      ELSE
C
C        CONVERSION CARACTERES => NUMERIQUE
C        ----------------------------------
C        LE TABLEAU N'EXISTE PAS . IL EST DECLARE EN MC
         NBVATC(LHTMS) = 128
         CALL TNMCDC( 'MOTS' , NBVATC(LHTMS) , MCTAMS(LHTMS) )
         NOTAMS(LHTMS) = 0
         MNTAMS(LHTMS) = MCTAMS(LHTMS)
         NBLGRC(NRERR) = 1
         KERR(1) =  ' LECTURE FICHIER DU TABLEAU: '//KNOMTS
         CALL LERESU
C
C        LE NOM DU TABLEAU DESCRIPTEUR DE CE TABLEAU TMS CARACTERE
C        LE TMS CARACTERE EST RANGE COMME SUIT
C        NOM_TMS DATE NOTD
C        SAUVEGARDE DU POINTEUR SUR LE DEBUT DU BUFFER DU FICHIER
C        ATTENTION: LA DATE ET NUMERO DE TD DOIVENT ETRE DANS
C                   LE MEME BUFFER
         I = LCFICA
C        LECTURE DE DATE
         CALL FICACD( NFFICA,MCN( MCTAMS(LHTMS)+LDTS(LHTMS)) , NRETOU )
         IF( NRETOU .NE. 0 ) RETURN
C        LE DECALAGE DU A UN REEL DOUBLE PRECISION
         LDTS(LHTMS) = LDTS(LHTMS) + MOTVAR( 6 )
C        LECTURE DE NOTD NUMERO DU TABLEAU DESCRIPTEUR
         CALL FICACE( NFFICA,MCN( MCTAMS(LHTMS)+LDTS(LHTMS)) , NRETOU )
         IF( NRETOU .NE. 0 ) RETURN
         NOTADS(LHTMS) = MCN( MCTAMS(LHTMS)+LDTS(LHTMS))
C        RESTAURATION DU POINTEUR SUR LE DEBUT DU BUFFER DU FICHIER
C        ATTENTION: LA DATE ET NUMERO DE TD DOIVENT ETRE DANS
C                   LE MEME BUFFER
         LCFICA = I
C        REDEMARRAGE A LA DATE
         LDTS(LHTMS) = 0
      ENDIF
C
C     LE POINTEUR SUR LE DERNIER CARACTERE STOCKE DANS KFICA
C     LCFICA EST INITIALISE A L'EXTERIEUR DE CE SP
C
...
C           NOM DU TD ET DU TS
     ...
     %                 , NB )
            IF( NB .EQ. 0 ) THEN
               NBLGRC(NRERR) = 3
               KERR(1) ='NCTSTD: FUSION IMPOSSIBLE DES NOMS'
...
C              OUVERTURE
               ...
               IF( MNTAMS(LHTMS) .LE. 0 ) THEN
                  NBLGRC(NRERR) = 2
                  KERR(1) = 'NCTSTD: TMS NON OUVRABLE'
                  KERR(2) = NOMTS(LHTMS)
                  CALL LEREUR
                  RETURN
               ENDIF
...
C                 ...........................................................
                  IF( LIRECR .EQ. 0 ) THEN
C                    MISE SUR LE FICHIER DE CET IDENTIFICATEUR
                     CALL NCTYVA( NFFICA , NBIDEN(LHTMS) , NRETOU )
                  ELSE
C                    CONVERSION DES CARACTERES EN VALEUR NUMERIQUE
                     CALL CNTYVA( NFFICA , NBIDEN(LHTMS) , NRETOU )
                  ENDIF
...
C
      IF( LIRECR .EQ. 0 ) THEN
C
C        ON VIDE LE BUFFER KFICA SUR LE FICHIER CARACTERE
C        ------------------------------------------------
         CALL FICAWR( NFFICA )
C        UN TMS S'ETEND SUR UN NOMBRE ENTIER D'ENREGISTREMENTS
C        2 TMS NE PEUVENT PARTAGER UN MEME ENREGISTREMENT
C
      ELSE
C
C        GENERATION EFFECTIVE DU TMS
C        ---------------------------
C        RECHERCHE DU LEXIQUE PERE DE KNOMTS
         CALL LXNTPO( NOMTS(LHTMS) , NTLXP , MNLXP , N1 , N2 )
         IF( NTLXP .LE. 0 ) THEN
            NBLGRC(NRERR) = 2
...
C           DESTRUCTION
         ...
         ENDIF
C
C        LE TABLEAU EXISTE-T-IL ?
         CALL LXNMOU( NTLXP  , NOMTS(LHTMS)(N1:N2) , 'TAMS' ,
     %                NOTAMS(LHTMS) , MNTAMS(LHTMS) )
         IF( NOTAMS(LHTMS) .GT. 0 ) THEN
C           OUI : IL EST DETRUIT POUR MIEUX ETRE RECONSTRUIT
            CALL LXNMDS( NTLXP  , NOMTS(LHTMS)(N1:N2) )
         ENDIF
C
C        ICI LE TABLEAU TMS N'EXISTE PAS OU PLUS
C        DECLARATION DU TABLEAU TMS
         NBMOTS = LDTS(LHTMS)
         CALL LXNMDC( NTLXP , NOMTS(LHTMS)(N1:N2) , 'TAMS' ,
     %                'MOTS', NBMOTS , NRETOU )
C        OUVERTURE DU TABLEAU TMS
         CALL LXNMOU( NTLXP  , NOMTS(LHTMS)(N1:N2) , 'TAMS' ,
     %                NOTAMS(LHTMS) , MNTAMS(LHTMS) )
