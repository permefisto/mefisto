      SUBROUTINE RECTTXSU( NUR, TEXTE, NETEXT, NATEXT, NUMERC )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C NUMERC : NUMERO DE LA CASE A SURLIGNER EN JAUNE
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C AUTEUR : PERRONNET ALAIN  ANALYSE NUMERIQUE UPMC PARIS        MAI 1994
C AUTEUR : PERRONNET ALAIN LJLL UPMC & Saint PIERRE DU PERRAY AVRIL 2009
C23456---------------------------------------------------------------012
...
C           ORIENTATION VERS LE MENU CORRESPONDANT
          ...
          ELSE
            ...
            NBLGRC(NRERR) = 2
            IF( LANGAG .EQ. 0 ) THEN
               KERR(1) = 'RECTTXSU : MAUVAISE VALEUR DE LASOPS'
               KERR(2) = '         ARRET DU DESSIN POSTSCRIPT'
            ELSE
               KERR(1) = 'RECTTXSU : BAD VALUE of LASOPS'
...
C
C              LA COULEUR DES CARACTERES
               CALL XVCOULEUR( NKRECT( NRMENU ) )
C
               IF( NUMERC .EQ. I ) THEN
C                 LA CASE A TRACER EST A SURLIGNER EN JAUNE
                  CALL XVCOULEUR( NCJAUN )
                  CALL XVFRECTANGLE( XRECT(NRMENU)+ECARLR(NRMENU)-1,
     %                           NY-DYLGRC(NRMENU)+2*ECARLR(NRMENU)+1,
     %                           DXRECT(NRMENU)-2*ECARLR(NRMENU)+NBEPO4,
     %                           DYLGRC(NRMENU)-ECARLR(NRMENU)-1 )
C                 LA COULEUR DES CARACTERES DE LA CASE SURLIGNEE
