...
| 2026-05-06 | Plan 9-08 cross-tag bootstrap attempt | DEFERRED-CARRY-FORWARD | Phase 9 Plan 9-08 attempted the cross-tag worktree procedure (RESEARCH §Pattern 3) to autonomously bootstrap all 3 goldens. The cross-tag worktree (`/tmp/mefisto-pre-retire` from `v1.0-pre-retire` tag, SHA `53bdb5b`) built the X11 backend successfully (`bin/cbl_tout` exit 0; 13 `pp/pp*` binaries). Two pre-existing Phase-7 design defects then blocked golden production: **(1)** scene01_driver.f calls `XVCHARGEFONTE('Courier', 7, 12, 10, 0)` with 5 args including a string literal, but `xvuelc.c:1474` declares the C entry as `(int *nofont0, int *nofont, int *largpx, int *hautpx)` — 4 INTEGER args. The string `'Courier'` is dereferenced as `*nofont0` (= `0x436F7572`), causing the function to call `XFreeFont` on a NULL pointer, then `XTextExtents` on the same NULL — SIGSEGV. The driver was committed in Phase 7 Plan 06 but **has never been run end-to-end**. **(2)** testa/wave and testa/cavity2d are interactive multi-module pipelines: `INITIER` (interactive ppinit, 4-5 prompts) → `MAILLER project.mesh` (interactive ppmail prompts) → `FLUIDER project.wave` (interactive ppflui prompts) → `bin/convertepsgif`. The plan's `testa/wave/wave.iexrr` + `wave.iexsr` files do not exist; the testa data files (`wave.mesh`, `wave.wave`) are MEFISTO project-scripts read AFTER the interactive INITIER stage. `echo "wave" \| INITIER` produces `Fortran runtime error: End of file` at the first ppinit READ past the project name. Empirical evidence + recommended fix paths recorded in `.planning/phases/09-retire-x11-backend/09-08-DEVIATIONS.md`. **The 3 rows above remain DEFERRED with refined rationale; no goldens were committed (committing a partial or wrong-bytes EPS would flip QSKIP → FAIL, strictly worse than DEFERRED).** Phase 9 RETIRE-04 already shipped; the X11 backend is GONE from main, so this is a permanent carry-forward unless source-repair (option 1 in 09-08-DEVIATIONS.md §Recommendation) is undertaken in a future Phase-7-scope plan. |
