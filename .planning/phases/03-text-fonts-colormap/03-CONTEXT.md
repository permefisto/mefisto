...
- **D-13:** `norgb[256]` is **preserved as storage only** for ABI compatibility (`xvrecuprgbdec_` may write to it in some callers; TBD by planner during RESEARCH phase). Phase 3 does not use `norgb` for any real lookup — Qt has no X11 Pixel equivalent. If no planner-identified caller writes to or reads from `norgb`, it can be dropped in Phase 3. Otherwise, stub with `norgb[i] = i` (identity).
...
- **D-15:** `xvCouleursImposees_` body: takes an array of RGB triples from Fortran, writes them into `red[]`/`green[]`/`blue[]` starting at index 0, marks all written indices dirty. Does not invalidate `foreground_` — if `*foreground_` index is in the range overwritten, the next drawing primitive picks up the new color on `palette_cache_[*foreground_]` rebuild. Matches legacy "`xvCouleursImposees_` loads a hardcoded palette from a default table".
- **D-16:** `xvStockeRGBtoColormap_(idx, r, g, b)` body: bounds-check `*idx`, write `red[*idx] = *r; green[*idx] = *g; blue[*idx] = *b; palette_cache_dirty_[*idx] = true;`. No side effects, no painter touch. Same template applies to `xvColormapToRGB_(idx, r, g, b)` (read side): bounds-check, `*r = red[*idx]; *g = green[*idx]; *b = blue[*idx];`.
- **D-17:** `xvactivervb_` body: legacy semantics = "activate this RGB triple for next draws, bypassing the indexed palette". Phase 3 implementation: constructs a transient `QColor::fromRgbF(*r, *g, *b)`, writes it into `state_->foreground_` directly, calls `applyPen()`. Does NOT write into `red/green/blue[]` arrays — `xvactivervb_` is one-shot, not a palette edit. No painter begin/end.
