...
- **nlsecu TRUNCATED-CAPTURE on all 4 Qt cells** — ppnlse_qt offscreen+BATCH_X11 deadlock at startup (10 log lines emitted, no NLSER banner reached even at 240 s; reproduced in Plans 1/3/4 with TIME=2 / TIME=0.2 truncations); canonical TIME=20 (2000 steps) ~hour-scale on this hardware exceeds 60 s harness budget. Mitigation per user decision: TIME=0.01 (X11 baseline) / TIME=0.1 (X11-OMP, Qt-OMP) workspace truncations + MAILLER-prereq fallback (Qt 1x, Qt 2x). Resolution requires either (i) Phase 9 candidate code-side fix to ppnlse_qt offscreen+BATCH_X11 deadlock, or (ii) explicit waiver-by-rationale in this CHECKLIST.md.
  - Override accepted: ☑ (i) Phase-9-deferred-fix — dricoco 2026-05-05. ppnlse_qt offscreen+BATCH_X11 deadlock is a real defect; documented as Phase 9 candidate. Truncated-capture evidence is sufficient for v1 ship gate (mesher prereq frame proves ABI link works; OMP scheduling proven via TIME=0.1 captures). Phase 9 Plan 9-07 Task 1 (diagnose) classified as case (c) long-runtime; no code defect — see `.planning/phases/09-retire-x11-backend/evidence/ppnlse-diagnose.md`. Production-harness arg-form (`pp/ppnlse nlsecu.iexrr` per `bin/ab_sweep_phase8.sh` line 153) reaches the `Mefisto-NLSER` banner in 17 log lines within 60s on FRESH binaries; the residual long-runtime symptom (TIME=20 / Step=0.01 → 2000 steps ~hour-scale) is the genuine and only constraint, mitigated by the Phase 8 TIME-truncation workspace work-around.
