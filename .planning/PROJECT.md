...
---
*Last updated: 2026-04-30 after Phase 6.5 (nonlinear-nlse-menu-wiring) — UX-05 (per-module menu wiring) Complete across all 5 solver modules: 6.1 mail (Mesh, 211 rows), 6.2 elas (Solve, 102), 6.3 flui (Fluid, 161), 6.4 ther (Thermal, 209), 6.5 nlse (Nonlinear, 80). All 5 modules expose strong-symbol `register<Mod>Actions_stub_` with force-link keepalive references in `xvue_qt_app.cpp` (alphabetical: elas/flui/mail/nlse/ther). Each `pp<mod>_qt` shows top-level menu bar with module-specific second slot (`{File, Mesh|Solve|Fluid|Thermal|Nonlinear, View, Help}`) and `CALL XVUE_MODULE_INIT('<mod>')` hook in `prpr/pp<mod>.f`. Help-allowlist split per module (`{97;}` for flui/nlse, `{98;}` for ther). Aggregate Qt sweep at phase end: 152 pass / 1 fail / 2 skip (1 fail = pre-existing `testPerModuleGroupIsolation`, deferred). ABI frozen at 58 symbols. `AUDIT_STRICT_ICONS=1` validation clean across all 5 audits. Phase 7 (Image, GIF, and PostScript export) is next.*
