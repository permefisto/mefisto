...
---
*Last updated: 2026-04-11 after Phase 1 (window-shell) completion — `XvueApp`/`XvueWindow`/`XvueCanvas`/`XvueState` stood up; 7 SHELL entry-point bodies (`xvinitgraphique_`, `xvfermer_`, `xvpxecran_`, `xvmmecran_`, `xvfond_`, `xvinfo_`, etc.) implemented on real Qt 6; `XVUE_QT_ASSERT_MAIN_THREAD()` retrofitted into all 57 extern "C" stubs; `verify_no_exec` CMake guard enforces SHELL-03; `pp/ppxvtest0_qt` opens, reopens, and exits cleanly (visually confirmed, HiDPI `QT_SCALE_FACTOR=2` validated). QApplication is deliberately leaked at process exit (D-08 revised) and `xvinitgraphique_` pumps events until `isExposed()` (D-01 revised) — see `.planning/debug/resolved/phase-01-xvtest0-teardown-segfault.md`. Requirements SHELL-01..SHELL-07 closed. Phase 2 (drawing primitives) is next.*
