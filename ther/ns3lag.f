      SUBROUTINE NS3LAG( DeltaT, NUTYEL, X,      PENALI, NBJEUX, JEU,
     ...
     %                   BE )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :  NLSE: CALCUL DU SECOND MEMBRE DES EF 3D
C -----        LAGRANGE ISOPARAMETRIQUES EN SCHEMA IMPLICITE EN TEMPS
C              c'est a dire
C REAL PART:
C [M(Rho)] {Wn} + [N(DeltaT beta,V,W)] {V} - {DeltaT FOmegaR(tn+1,V,W)}
C
C IMAGINARY PART:
C [M(Rho)] {Vn} - [N(DeltaT beta,V,W)] {W} + {DeltaT FOmegaI(tn+1,V,W)}
C
C ENTREES:
C --------
...
C
     ...
     %                  F(NPI,3),
     %                  POIDEL(NPI)
      DOUBLE PRECISION  BE(NBPOLY,2), PENALI
      REAL              X(NBPOLY, 3)
...
C        TEMPEL: PARTIE REELLE     A tn+1 AU POINT D'INTEGRATION L de L'ONDE
C        ONDEPI: PARTIE IMAGINAIRE A tn+1 AU POINT D'INTEGRATION L de L'ONDE
C        TEMPELn:PARTIE REELLE     A tn   AU POINT D'INTEGRATION L de L'ONDE
C        ONDEPIn:PARTIE IMAGINAIRE A tn   AU POINT D'INTEGRATION L de L'ONDE
C        TEMPEL0:PARTIE REELLE     A t0   AU POINT D'INTEGRATION L de L'ONDE
C        ONDEPI0:PARTIE IMAGINAIRE A t0   AU POINT D'INTEGRATION L de L'ONDE
C        VALEUR V(L)+iW(L) DE L'ONDE INTERPOLEE AU POINT D'INTEGRATION L
         Vtm(L) = TEMPEL
         Wtm(L) = ONDEPI
         Vtn(L) = TEMPELn
         Wtn(L) = ONDEPIn
...
C        ----------------
         ...
         IF( MN .GT. 0 ) THEN
            CALL REMASS( 4, NOOBVO, 3, XYZ, MN, MASSE(L) )
         ENDIF
C
...
C              DeltaT [N(Beta)] V  + [M(Dmasse)] Wn - DeltaT FOmegaR
               BE(I,1) = BE(I,1) + POLY(I,L) *
     %          (MASSE(L)*Wtn(L) + (BETA(L)*Vtm(L) - FORCE(1,L))*DeltaT)
C
C              [M(Dmasse)] Vn -  DeltaT [N(Beta)] W + DeltaT FOmegaI
               BE(I,2) = BE(I,2) + POLY(I,L) *
     %          (MASSE(L)*Vtn(L) + (FORCE(2,L) - BETA(L)*Wtm(L))*DeltaT)
C
            ENDDO
         ENDIF
C
