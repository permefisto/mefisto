      SUBROUTINE NLSITMGKG(PENALI, NDIM,   MNXYZN,
     ...
     %                     NBPTAF, IERR  )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    GP: CALCUL DE LA MATRICE GLOBALE [KG(Alfa,N(|U0|**2)] nxn
C -----    DES ELEMENTS FINIS LAGRANGE 1D OU 2D OU 3D DE DEGRE 1 OU 2
C
C          [KG] = [ Rho/dt + Alfa LAPLACIEN - N(|U0|**2) ]
C          [MG] = Integrale tP P dX
C
C          GP PROBLEM: COMPUTATION OF THE GLOBAL MATRIX nxn
...
C MG     : MATRICE GLOBALE DE MASSE SUR LE MAILLAGE
...
C
      ...
      DOUBLE PRECISION  KG(NBRDKG), MG(NBRDKG)
C
...
C     INITIALISATION DE LA MATRICE GLOBALE DU SYSTEME
      ...
      ELSE
         WRITE(IMPRIM,20002)
      ENDIF
10002 FORMAT(/'CONSTRUCTION de la MATRICE GLOBALE [KG(Alfa,N(|U0|**2)]')
20002 FORMAT(/'CONSTRUCTION of [KG(Alfa,N(|U0|**2)] GLOBAL MATRIX ')
C
C     INITIALISATION A ZERO DE LA MATRICE GLOBALE
C     -------------------------------------------
      CALL AZEROD( NBRDKG, KG  )
      CALL AZEROD( NBRDKG, MG  )
C
...
C        ERREUR
 1       NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'ERREUR NLSITMGKG: TYPE EF '// NOMELE(1)
     %           // NOMELE(2) //' NON PROGRAMME'
         ELSE
            KERR(1) = 'ERROR NLSITMGKG: FE TYPE '// NOMELE(1)
...
cccC           [Ke] <= [Me]/dt +[Ke]
cccC           ---------------------
ccc 70         MNM = ( MNCAEL-1 ) / MOREE2
ccc            MNK = ( MNTAEL-1 ) / MOREE2
ccc            DO J = 1, NBPOLY*(NBPOLY+1)/2
ccc               DMCN( MNK+J ) = DMCN( MNM+J ) / DELTAT + DMCN( MNK+J )
ccc            ENDDO
C
...
C              ASSEMBLAGE DE ME DANS LA MATRICE MG PROFIL GLOBALE
               CALL ASMEPC( NBPOLY, MCN(MNNODL),
     %                      NCODSK, MCN(MNCAEL), MCN(MNCAEL),
     %                      NCODSK, MCN(MNLPLI), MG      )
C
...
C              ASSEMBLAGE DE ME DANS LA MATRICE MG MORSE GLOBALE
               CALL ASMEGC( NBPOLY, MCN(MNNODL),
     %                      NCODSK, MCN(MNCAEL), MCN(MNCAEL),
     %                      NCODSK, MCN(MNLPLI), MCN(MNLPCO), MG )
