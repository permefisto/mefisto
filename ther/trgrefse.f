      SUBROUTINE TRGREFSE( KNOMOB, NTLXOB, NOPROJ,
     %                     NBTYEL, MNELEM, NDPGST,
     %                     NBCOOR, NBPOI,  XYZPOI,
     %                     MODECO, NCAS,   NOPT,
     %                     GRADMX, GRADCM, CMFLEC, CMPGRA )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    TRACE DES GRADIENTS DES EF SECTIONNES PAR UN
...
C
C     TABLEAU DES XYZ DIRECTION + LONGUEUR DES FLECHES GRADIENTS
...
C
         IF (NORM .LT. 1E-8) THEN
            NBLGRC(NRERR) = 2
            IF( LANGAG .EQ. 0 ) THEN
               KERR(1) = 'TRGREFSE: VECTEUR NORMAL AU PLAN NUL'
               KERR(2) = '          VEUILLEZ RECOMMENCER'
            ELSE
               KERR(1) = 'TRGREFSE: NULL NORMAL VECTOR'
...
C        MISE A JOUR DU PIXEL INITIAL DE LA SOURIS
      ...
      ENDIF
C
C     CONSTRUCTION DES VECTEURS GRADIENTS
...
C        L'ADRESSE MCN DU TABLEAU 'NPEF' POUR CE TYPE D'EF
         ...
         ENDIF
C
C        TRACE DES GRADIENTS DANS LES EF SECTIONNES
C        ------------------------------------------
...
C           LISTAGE DES VECTEURS GRADIENTS
            CALL VEGRAD( NCAS,   CMPGRA,
     %                   NEF, NBELEM, NBPIEF, NDIMES, NBJECA,
     ...
     %                   NBAREF, MXAREF, MCN(MNAREF), NOPTEF, IERR )
            IF( IERR .NE. 0 ) THEN
               IF( LANGAG .EQ. 0 ) THEN
                  IF( IERR .EQ. 1 ) THEN
                  WRITE(IMPRIM,*)'TRGREFSE: AUGMENTER MXFLEC=',MXFLEC
                  ELSE
                  WRITE(IMPRIM,*)'TRGREFSE: AUGMENTER MXAREF=',MXAREF
                  ENDIF
               ELSE
                  IF( IERR .EQ. 1 ) THEN
                  WRITE(IMPRIM,*)'TRGREFSE: AUGMENT MXFLEC=',MXFLEC
                  ELSE
                  WRITE(IMPRIM,*)'TRGREFSE: AUGMENT MXAREF=',MXAREF
                  ENDIF
               ENDIF
               GOTO 300
            ENDIF
...
C     FIN DU TRACE
      IF( MODECO .EQ. 1 ) THEN
         ...
         IF( LANGAG .EQ. 0 ) THEN
      KNOM = 'CAS      GRADIENTS dans un PLAN de SECTION au TEMPS='
     %    // KNOM(1:14)
         ELSE
      KNOM = 'CASE     GRADIENTS in a PLANE SECTION at TIME='
     %    // KNOM(1:14)
         ENDIF
         WRITE( KNOM(5:8), '(I4)' ) NCAS
      ELSE
         IF( LANGAG .EQ. 0 ) THEN
         KNOM = 'VALEUR PROPRE               : GRADIENTS du VECTEUR PROP
     %RE dans un PLAN'
         ELSE
         KNOM = 'EIGENVALUE                  : GRADIENTS of the EIGENVEC
     %TOR in a PLANE'
         ENDIF
