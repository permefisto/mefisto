      SUBROUTINE DFTHER( KNOMOB, NBJEUX, NERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    DEFINIR LES CARACTERISTIQUES PHYSIQUES DE LA THERMIQUE
C -----    D'UN OBJET ET LES SOURCES EXERCEES
C
C          INPUT DATA of the THERMAL CHARACTERISTICS of EVERY MATERIAL
C          INPUT DATA of the THERMAL BOUNDARY CONDITIONS
C
C ENTREE :
C --------
C KNOMOB : NOM DE L'OBJET A THERMIQUER    (WORK OBJECT NAME)
C NBJEUX : NOMBRE DE JEUX DE DONNEES      (INPUT GAME NUMBER)
C          =1 par DEFAUT ou CAS STANDARD
C          >1 si POLYNOME DE VALEURS PROPRES  NBJEUX=DEGRE+1 du POLYNOME
C
C SORTIE :
C --------
...
C2345X7..............................................................012
      ...
      CHARACTER*12      LISTE2(LLIST2)
      DATA LISTE1/'MASSE',           'CONDUCTIVITE',  'DILATATION',
     %            'SOURCE',          'VITESSEFLUIDE', 'COEFTEMPERATURE',
     %            'CHALEURMASSIQUE', 'TEMPERINIT',    'VITESSEINIT',
     %            'COEFBOUSSINESQ' /
      DATA LISTE2/'ECHANGE', 'SOURCE', 'CONTACT', '    ',
     %             '    ',   '    ',   '    ',
     %             'TEMPERINIT', 'VITESSEINIT'/
C
      IF( LANGAG .EQ. 0 ) THEN
      ...
      ENDIF
10000 FORMAT(/1X,80('*')/
     %' LECTURE DES DONNEES THERMIQUES DE L''OBJET ',A/1X,80('*'))
20000 FORMAT(/1X,80('*')/
     %' READING of INPUT HEAT DATA of the OBJECT ',A/1X,80('*'))
C
C     VERIFICATIONS
C     =============
...
C     =================================================
      ...
      IF (NDOUNO .GT. 0) GOTO 101
C
C     NON : RESOLUTION CLASSIQUE
C     NO  : CLASSIC FORMULATION
C     ==========================
...
C        variable COEFTHER 'Operator divAgrad INPUT' entier
C        ( 1 : 'Rho MASS Density'
C        2 : '[A] HEAT CONDUCTIVITY'
C        3 : 'DILATATION & TEMPERATURE REFERENCE'
C        4 : 'FOmega INTERIOR HEAT SOURCE'
C        5 : 'Vf Coefficient of GradTEMPERATURE'
C        6 : 'CT Coefficient of TEMPERATURE'
C        7 : 'C  MASSIC HEAT Density'
C        8 : 'INITIAL TEMPERATURE'
C        9 : 'Vo INITIAL WAVE SPEED'
C       10 : 'CoBOUS Coefficient de BOUSSINESQ' ) ;
C        ==================================================
         CALL SDDEF2( NUOBJE, NYOB, NUOB, LISTE1, LLIST1,
     %               'THERMIQUE','coefther', NBJEUX )
 100  CONTINUE
C
...
C        ===============================================================
         DO 300 K = 1, NBJEUX
           ...
           KERR(1)='ATTENTION: Entree du JEU de DONNEES Numero='//KNOJEU
            ...
            CALL LERESU
C
C           LE NOM DU TABLEAU A ENTRER EST UNE OPTION de cl_ther
C           THE NAME of the ARRAY to INPUT Is an option of cl_ther
C           variable CL_THER 'div A grad Boundary Condition' entier
C           ( 1 : 'g HEAT TRANSFER COEFFICIENT wall<->fluid'
C           , 2 : 'FGamma SOURCE  Boundary IMPOSED FLUX'
C           , 3 : 'Theta_D CONTACT IMPOSED TEMPERATURE'
C           , 8 : 'Theta_0 INITIAL TEMPERATURE'
C           , 9 : 'Vo INITIAL WAVE SPEED' ) ;
 210        CALL LIMTCL( 'cl_ther' , NMTCL )
...
C                          DESCRIPTOR ARRAY
               IF( IERR .NE. 0 .OR. MCN(MNTS+WTCONT) .EQ. 0 ) THEN
...
C     ERREUR ERROR
 9900 NERR = 1
      IF( INTERA .GE. 3 ) RETURN
      CALL ARRET(100)
C
C     2D RESOLUTION PAR SOUS-DOMAINES
C     SOLUTION BY SUB-DOMAINS IN 2D
C     ===============================
 101  CALL SDDFTH( KNOMOB, LISTE1, LLIST1, LISTE2, LLIST2,
     %            'THERMIQUE','coefther','cl_ther', IERR)
C
      RETURN
      END
