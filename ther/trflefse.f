      SUBROUTINE TRFLEFSE( KNOMOB, NTLXOB, NOPROJ,
     %                     NBTYEL, MNELEM, NDPGST,
     %                     NBCOOR, NBPOI,  XYZPOI,
     %                     MODECO, NCAS,   CMFLUX )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    TRACE DES FLUX NORMAUX DES EF SECTIONNES PAR UN
...
C CMFLUX : NOMBRE DE CM POUR L'UNITE DE FLUX
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C     VARIABLES PERSO
      ...
      REAL              XYZ0(3), XYZ(3)
      REAL              COPLAN0, COPLAN, COPLANMIN, COPLANMAX, PROSCR
      REAL              XYZPLAN(3,4)
      INTEGER           NOPTEF(64)
...
C
C     TABLEAU DES XYZ DIRECTION + LONGUEUR DES FLECHES FLUX NORMAUX
      MXFLEC = 6 * NBELFIN( NBTYEL, MNELEM ) + 1024
ccc      MXFLEC = 12 * NBARFR
      CALL TNMCDC( 'REEL', MXFLEC*7, MNXYZF )
...
C
         IF (NORM .LT. 1E-8) THEN
            NBLGRC(NRERR) = 2
            IF( LANGAG .EQ. 0 ) THEN
               KERR(1) = 'TRFLEFSE: VECTEUR NORMAL AU PLAN NUL'
               KERR(2) = '          VEUILLEZ RECOMMENCER'
            ELSE
               KERR(1) = 'TRFLEFSE: NULL NORMAL VECTOR'
...
C        MISE A JOUR DU PIXEL INITIAL DE LA SOURIS
      ...
      ENDIF
C
C     CONSTRUCTION DES VECTEURS FLUX NORMAUX
...
C        L'ADRESSE MCN DU TABLEAU 'NPEF' POUR CE TYPE D'EF
         ...
         ENDIF
C
C        TRACE DES FLUX NORMAUX AUX POINTS DES FACES DES EF
C        --------------------------------------------------
...
C        OUVERTURE DU TABLEAU
         ...
         IF( NTFLUX .LE. 0 ) THEN
            NBLGRC(NRERR) = 1
            IF( LANGAG .EQ. 0 ) THEN
               KERR(1) = 'OBJET SANS TABLEAU FLUX'
            ELSE
               KERR(1) = 'OBJECT WITHOUT COMPUTED FLUXES'
            ENDIF
            CALL LEREUR
            GOTO 9000
         ENDIF
...
C           LISTAGE DES VECTEURS FLUX NORMAUX
            CALL VEFLUX( NCAS,   CMFLUX,
     %                   NBPNFX, NEF, NBELEM, RMCN(MNCOPN), MCN(MNFLNP),
     %                   NBFLEC, MXFLEC, RMCN(MNXYZF), CMMXFL,
     %                   NBAREF, MXAREF, MCN(MNAREF), NOPTEF, IERR )
            IF( IERR .NE. 0 ) THEN
               IF( LANGAG .EQ. 0 ) THEN
                  IF( IERR .EQ. 1 ) THEN
                  WRITE(IMPRIM,*)'TRFLEFSE: AUGMENTER MXFLEC=',MXFLEC
                  ELSE
                  WRITE(IMPRIM,*)'TRFLEFSE: AUGMENTER MXAREF=',MXAREF
                  ENDIF
               ELSE
                  IF( IERR .EQ. 1 ) THEN
                  WRITE(IMPRIM,*)'TRFLEFSE: AUGMENT MXFLEC=',MXFLEC
                  ELSE
                  WRITE(IMPRIM,*)'TRFLEFSE: AUGMENT MXAREF=',MXAREF
                  ENDIF
               ENDIF
               GOTO 300
            ENDIF
...
C     FIN DU TRACE
      IF( MODECO .EQ. 1 ) THEN
         ...
         IF( LANGAG .EQ. 0 ) THEN
      KNOM = 'CAS      FLUX NORMAUX dans un PLAN de SECTION au TEMPS='
     %    // KNOM(1:14)
         ELSE
      KNOM = 'CASE     NORMAL FLUXES in a PLANE SECTION at TIME='
     %    // KNOM(1:14)
         ENDIF
         WRITE( KNOM(5:8), '(I4)' ) NCAS
      ELSE
         IF( LANGAG .EQ. 0 ) THEN
         KNOM = 'VALEUR PROPRE               : PLAN de SECTION du VECTEU
     %R PROPRE'
         ELSE
         KNOM = 'EIGENVALUE                  : SECTION of the EIGENVECTO
     %R in a PLANE'
         ENDIF
         WRITE( KNOM(15:29), '(G14.6)' ) TEMPS
      ENDIF
...
C     ====================================
 9000 IF( MNBARY .GT. 0 ) CALL TNMCDS( 'REEL',  MXBARY,   MNBARY )
      ...
      IF( MNXYZF .GT. 0 ) CALL TNMCDS( 'REEL',  MXFLEC*7, MNXYZF )
      END
