      SUBROUTINE NSB2LAG(NUELEM, NONOEF, Omega,
     ...
     %                   BE )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :  NLSE: CALCUL DU SECOND MEMBRE BE D'UN EF AXISYMETRIQUE OU 2D
C -----        LAGRANGE ISOPARAMETRIQUE A L'INSTANT TEMPS
C              c'est a dire
...
ccc   DOUBLE PRECISION  VN(2)
      ...
      DOUBLE PRECISION  Rho(MAXPTI), NLCOEF(MAXPTI), FORCE(2,MAXPTI),
     %                  Vtm(MAXPTI),Wtm(MAXPTI),Vtn(MAXPTI),Wtn(MAXPTI),
...
C            VECTEUR(3) DE VITESSE ANGULAIRE AU 1-ER POINT D'INTEGRATION
             CALL REVIAN( 3, NOOBSF, F1(1), F2(1), 0D0, MN,  Omega )
C            POUR UNE SEULE COMPOSANTE DONNEE UTILE Omega(1)=Omega/Z
          ELSE
C            PAS DE ROTATION
             Omega(1) = 0D0
             Omega(2) = 0D0
...
C        ----------------
         ...
         ENDIF
C
C        COEFFICIENT DU TERME NON LINEAIRE AU TEMPS ACTUEL  N(Vn+1m**2+Wn+1m**2)
C       -COEFFICIENT DU TERME NON LINEAIRE AU TEMPS INITIAL N(V0**2+W0**2)
C        -----------------------------------------------------------------------
...
C        Omega ( x dVn+1m/dy - y dVn+1m/dx )(bl)
         F1dYF2dXV(L) = Omega(1) * SV * S
C        Omega ( x dWn+1m/dy - y dWn+1m/dx )(bl)
...
C              + ( N(V0**2+W0**2) - N(Vn+1m**2+Wn+1m**2) ) Vn+1m
     ...
     %                 * ( FORCE(1,L) + Rho(L) * (Wtm(L)-Wtn(L))
     %                    -F1dYF2dXW(L) -NLCOEF(L) * Vtm(L) )
C
C           BE2: Fi(tn+1)
C              - Rho/dt (Vn+1m-Vn) +OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )
C              + ( N(V0**2-W0**2) - N(Vn+1m**2+Wn+1m**2) ) Wn+1m
               BE(I,2) = BE(I,2) + POLY(I,L)
     %                 * ( FORCE(2,L) - Rho(L) * (Vtm(L)-Vtn(L))
     %                    +F1dYF2dXV(L) -NLCOEF(L) * Wtm(L) )
C
            ENDDO
C
...
C                   +Rho/dt Wn +OmegaZ ( x dWn+1m/dy - y dWn+1m/dx )
               BE(I,1) = BE(I,1) + POLY(I,L)
     %                 * ( NLCOEF(L) * Vtm(L) -FORCE(1,L)
     %                    +Rho(L) * Wtn(L) +F1dYF2dXW(L) )

C              BE2:  Fi(tn+1) + (N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2)) Wn+1m
C                    + Rho/dt Vn + OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )
               BE(I,2) = BE(I,2) + POLY(I,L)
     %                 * ( FORCE(2,L) - NLCOEF(L) * Wtm(L)
     %                    +Rho(L) * Vtn(L) +F1dYF2dXV(L) )
...
C                 NBCOFI LE NOMBRE DE COMPOSANTES FIXEES
                  DO I = 1, NBCOFI
C                    NU NUMERO DE LA COMPOSANTE FIXEE
