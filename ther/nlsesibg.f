      SUBROUTINE NLSESIBG( DeltaT, PENALI, NDIM,  MNXYZN, NBNOMA,
     %                     NBTYEL, MNNPEF, NDPGST,
     %                     MNTPOB, MNTAUX, NUMIOB, NUMAOB, MNDOEL,
     %                     MNX,    Utn,    Utm, 
     %                     BG,     IERR  )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : NLSE: TESTNL=6 CALCUL DU SECOND MEMBRE BG SCHEMA SEMI-IMPLICITE
C -----
C     REAL PART:
...
C          7 SI PB AXISYMETRIQUE AVEC X=RAYON NEGATIVE OU NUL
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
      INTEGER           NONOEF(20), NOOBSF(6), NOOBLA(12), NOOBPS(8)
...
C        ERREUR
 1       NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'ERREUR NLSESIBG: TYPE EF '// NOMELE(1)
     %           // NOMELE(2) //' NON PROGRAMME'
         ELSE
            KERR(1) = 'ERROR NLSESIBG: FE TYPE '// NOMELE(1)
...
C           -------------------------------------------------
            CALL NLSB2LAG( NUELEM, NONOEF, Omega,
     %                   DeltaT, D2PI, NOAXIS,
...
C           ----------------------------------
            CALL NLSB3LAG(NUELEM, NONOEF, Omega,
...
C           ----------------------------
 42         CALL NLSB2P1D(NUELEM, NONOEF, Omega,
...
C           LE SECOND MEMBRE ELEMENTAIRE
C           ----------------------------
            CALL NLSB3P1D(NUELEM, NONOEF, Omega,
