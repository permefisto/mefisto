      SUBROUTINE SDTRFL( NOPROJ, MODECO, KNOMOB, NTLXOB, NBTRFL, NBSD,
     %                   KNOMCO, NBTYEL, MNTOPO, MNELEM, MNPOGE, NDIM,
     %                   NCAS  , NOPT  , CMFLEC, CMPFLU,
     %                   FLUXMX, TIMES )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C     -----------------
 120  NOPT = 2
      NCVALS = 0
      CALL INVITE( 101 )
      CALL LIRRSP(NCVALS,FLUXMX)
      IF( NCVALS .EQ. -1 ) GOTO 100
 135  IF( FLUXMX .EQ. 0. ) THEN
         NBLGRC(NRERR) = 1
...
C        COULEUR NOIRE
      ...
      GOTO 100
C
C     EXECUTION DU TRACE DES FLUX
C     ---------------------------
 300  IF( NOPT .EQ. 1 .AND. CMFLEC .LE. 0. ) GOTO 9999
      IF( NOPT .EQ. 2 .AND. FLUXMX .LE. 0. ) GOTO 9999
      IF( NOPT .EQ. 3 .AND. CMPFLU .LE. 0. ) GOTO 9999
...
C        -------------------------------------------
         NBELFI = MCN(MNFLUX+WBELFX)
         CALL XVTYPETRAIT( NTLAFR )
         CALL XVEPAISSEUR( 1 )
         CALL TRAREF( NDIM,   MCN(MNPOGE+WNBPOI), MCN(MNPOGE+WYZPOI),
     %                NBELFI, MCN(MNPGEL) )
C
C        TRACE EFFECTIF DES FLECHES DES FLUX DE TEMPERATURE
C        --------------------------------------------------
C        LIGNES EPAISSIES
         CALL XVEPAISSEUR( 2 )
         CALL XVTYPETRAIT( LIGCON )
C
C        TRACE DES FLUX NORMAUX SUR LES FACES DES EF
C        -------------------------------------------
         KNOM = 'FLUXPT"' // NOMELE(2)
C        OUVERTURE DU TABLEAU
         CALL LXTSOU( NTLXOB, KNOM, NTFLUX, MNFLUX )
...
C        LES VARIABLES DU TMS 'FLUXPT"NMTYEL'
         ...
         MNCOPN = MNFLNP + MOTVAR(6) * NBPNFX * NBELFI * NBJECA
         CALL TRFL23( NDIM, NBELFI, NBPNFX, NBJECA,
...
C     FIN DU TRACE
      CALL TRFINS( 'FLUX NORMAL DE TEMPERATURE' )
C
...
C     ERREUR
 9999 WRITE(KERR(MXLGER-3)(1:13),'(I13)')   NOPT
      ...
      WRITE(KERR(MXLGER-1)(1:13),'(G13.5)') FLUXMX
