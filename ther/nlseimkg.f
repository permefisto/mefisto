      SUBROUTINE NLSEIMKG( DeltaT, PENALI, NDIM,   MNXYZN,
     ...
     %                     NBPTAF, IERR  )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : NLSE: SCHEMA IMPLICITE TESTNL=7 CALCUL DE LA MATRICE GLOBALE 2nx2n
C ----- KG = [-(-Alfa Laplac+N(V0**2+W0**2));  Rho/dt           ]
C            [ Rho/dt;              -Alfa Laplac +N(V0**2+W0**2)]
C       POUR DES ELEMENTS FINIS LAGRANGE 2D OU 3D DE DEGRE 1 OU 2
C
C       NLSE PROBLEM: COMPUTATION OF THE GLOBAL MATRIX 2n x 2n
...
C            LA DENSITE DE MASSE Rho EST INDEPENDANTE DE t et xyz
C            LA CONDUCTIVITE    Alfa EST INDEPENDANTE DE t et xyz
C            L'ONDE DOIT ETRE FIXEE EN CHACUN DES NOEUDS DIRICHLET
C                   A LA FOIS SUR SA PARTIE REELLE ET IMAGINAIRE
C                   AVEC DES VALEURS POUVANT ETRE DIFFERENTES
C
C ENTREES:
C --------
...
C
      DOUBLE PRECISION  PENALI, D2PI, DELTA, DeltaT, MATELE(820), UnSDt
ccc     DOUBLE PRECISION  ROTAEF(400)
      INTEGER           NODLNO(40)
...
C     INITIALISATION DE LA MATRICE GLOBALE DU SYSTEME
      ...
      ELSE
         WRITE(IMPRIM,20002)
      ENDIF
10002 FORMAT('Construction de la MATRICE GLOBALE [KG]')
20002 FORMAT('Construction of the GLOBAL MATRIX [KG]')
C
...
C        ERREUR
 1       NBLGRC(NRERR) = 1
         IF( LANGAG .EQ. 0 ) THEN
            KERR(1) = 'ERREUR NLSEIMKG: TYPE EF '// NOMELE(1)
     %           // NOMELE(2) //' NON PROGRAMME'
         ELSE
            KERR(1) = 'ERROR NLSEIMKG: FE TYPE '// NOMELE(1)
...
C
C           LA MATRICE ELEMENTAIRE DE MASSE EN 1D
C           -------------------------------------
C           MATRICE DE MASSE SYMETRIQUE NON DIAGONALE
...
C
C           LA MATRICE ELEMENTAIRE DE MASSE EN 2D OU EN AXISYMETRIE
C           ----------------------------------------------------------
C           EF 2D ET AXISYMETRIE SAUF TRIA 2P1D
C           MATRICE DE MASSE SYMETRIQUE NON DIAGONALE
            CALL TM2LAG( D2PI,        NOAXIS,    NBJEUX, JEU,
     %                   NBPOLY,      NPI,       MCN(MNPOL),
     %                   NOOBSF(1),   NUMIOB(3), NUMAOB(3), MCN(MNDEF3),
...
C           --------------------------------------------------------------
     ...
     %                   MCN(MNTHER),MCN(MNTHER),MCN(MNTAEL),IERR)
            IF( IERR .NE. 0 ) GOTO 9999
ccc
cccC           CALCUL DE LA MATRICE ELEMENTAIRE DUE A LA ROTATION
cccC           [ OmegaZ (x d/dy-y d/dx) ]
cccC           --------------------------------------------------
ccc            CALL TO2LAG( NUELEM, Omega,  D2PI,   NOAXIS, NBJEUX, JEU,
ccc     %                   NBPOLY, NPI,    POLY,
ccc     %                   NOOBSF,NUMIOB(3),NUMAOB(3),MCN(MNDEF3),
ccc     %                   MCN(MNF1),MCN(MNF2),MCN(MNPDEL),MCN(MNDP),
ccc     %                   ROTAEF )
...
C           ----------------------------------------
 51         CALL E13LAG( NBPOLY, NPI, MCN(MNPOID),
     ...
     %                   MCN(MNPDEL), MCN(MNDP), MCN(MNDFM1) )
C
C           LA MATRICE ELEMENTAIRE DE MASSE EN 3D
C           ----------------------------------------
...
C           -----------------------------------------
 42         CALL TM2P1D( RMCN(MNX), NBJEUX, JEU,
     %                   NOOBSF(1),NUMIOB(3),NUMAOB(3),
     %                   MCN(MNDEF3),NCODEM,MCN(MNCAEL) )
C
C           TRANSFORMATION MATRICE DIAGONALE EN MATRICE SYMETRIQUE
C           SUR ELLE MEME
            CALL DIASYM( 3, MCN(MNCAEL), MCN(MNCAEL) )
            NCODEM = 1
C
...
C           TRANSFORMATION MATRICE DIAGONALE EN MATRICE SYMETRIQUE
C           SUR ELLE MEME
            CALL DIASYM( 4, MCN(MNCAEL), MCN(MNCAEL) )
            NCODEM = 1
C
...
C
C           =======================================================
C           LA MATRICE ELEMENTAIRE AVEC PARTIE REELLE ET IMAGINAIRE
C           =======================================================
C           [-(-Alfa Laplac+N(V0**2+W0**2));          Rho/dt]
C           [   Rho/dt;         -Alfa Laplac +N(V0**2+W0**2)]
 70         CALL NLSEKeMe( NBPOLY,
     %                     1D0,    MCN(MNTAEL),
     %                     UnSDt,  MCN(MNCAEL),
     %                     MATELE )

ccc            si terme OmegaZ ( x d/dy - y d/dx ) dans la matrice KG
ccc            mais attention terme non symetrique!...
ccc 70         IF( Omega(1) .NE. 0D0 ) THEN
ccc               CALL NLSEKeMeRe( NBPOLY,
ccc     %                          1D0,    MCN(MNTAEL),
ccc     %                          UnSDt,  MCN(MNCAEL),
ccc     %                         -1D0,    ROTAEF,
ccc     %                          MATELE )
ccc            ENDIF


C           PASSAGE DE LA NUMEROTATION PAR BLOC A CELLE PAR NOEUD
C           =====================================================
            CALL NOBLNO( NBPOLY, 2, MCN(MNNODL), NODLNO )
C
