      SUBROUTINE SDDFTH( KNOMST , LISTE1 , LLIST1 , LISTE2 , LLIST2 ,
     %                   KPB    , KPBMTC , KPBMCL ,  IERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    DEFINIR LES CARACTERISTIQUES PHYSIQUES DE LA THERMIQUE
C -----    DE L'OBJET  KNOMST, DANS LE CAS D'UNE
C          RESOLUTION PAR SOUS-DOMAINES
C
C ENTREE :
C --------
C KNOMST : NOM DE L'OBJET A THERMIQUER
C
C SORTIES :
C ---------
...
C2345X7..............................................................012
      ...
      CHARACTER*(*)     LISTE1(1:LLIST1)
      CHARACTER*(*)     LISTE2(1:LLIST2)
...
C     LES LIGNES DE L'OBJET
      ...
      DO 200 NO = 1 , NBDOBJ
         ...
         KNOMTY = NMTYOB(NUTYOB)
         MNSD   = MNSD + 2
C        LE NOM DE L'OBJET AUX LIMITES
         CALL NMOBNU( KNOMTY , NUMEOB , KNOMLI )
C        LIGNES OU POINTS SEULEMENT
         IF (NUTYOB .EQ. 1 ) THEN
C           SON LEXIQUE
            CALL LXLXOU( NTPOIN , KNOMLI , NTLXOB , MNLXOB )
         ELSE IF (NUTYOB .EQ. 2 ) THEN
C           SON LEXIQUE
         ...
         IF( NTLXOB .LE. 0 ) THEN
            NBLGRC(NRERR) = 1
            KERR(1) = ' ERREUR : OBJET INCONNU ' // KNOMLI
            CALL LEREUR
            GOTO 9900
         ENDIF
...
C              LE CAS DES POINTS ET DES LIGNES
               ...
               KNOMTY = NMTYOB( NUTYOB )
               CALL NMOBNU( KNOMTY , NUMOBJ , KNOMOB )
C               WRITE (IMPRIM,1000) KNOMOB
C 1000          FORMAT(' SDDFTH : RECHERCHE DE ',A24)
               IF( NTOBRE .LE. 0 ) THEN
                  NBLGRC(NRERR) = 1
                  KERR(1) = ' ERREUR : OBJET INCONNU ' // KNOMOB
                  CALL LEREUR
...
C              ** ATTENTION L'IDENTIFICATION EST FAITE SUR LES SOMMETS ***
               CALL SDDEF5(NBLIMI,MCN(MNLIMI),
     %                     NUTYOB,NUMOBJ,NTOBRE,NTOBTR)
               IF (NTOBTR .GT. 0) THEN
C              L'OBJET EST RETROUVE : TRANSMISSION DES DONNEES AUX LIMITES
                  ...
                  LLISTE = 3
C                  WRITE (IMPRIM,2000)
C 2000             FORMAT(' L''OBJET EST RETROUVE')
