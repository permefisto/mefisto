      SUBROUTINE NLSEITIME( KNOMOB, MOREE2, NDIM, NDPGST, MNXYZN,
     ...
     %                     NTVECT, MNVECT, IERR )
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : TESTNL=9: L'equation de SCHRODINGER (NLSE)
C -----
C  i Rho dU(t,X)/dt - Alfa LAPLACIEN U(t,X) + N(|U(t,X)|**2) U(t,X)
C                   - i OmegaZ (x dU(t,X)/dy - y dU(t,X)/dx) = F(t,X)

C dont une variante est l'equation de GROSS-PITAEVSKII (GPE)
C  i Rho dU(t,X)/dt + LAPLACIEN U(t,X)/2 - ( V +Beta |U(t,X)|**2 ) U(t,X)
C                   - i OmegaZ (x dU/dy - y dU/dx) =F
C c-a-d dans NLSE, par identification, les donnees sont
C Alfa=-1/2,  N(|U(t,X)|**2)= -( V +Beta |U(t,X)|**2 )
C avec V=r**2/2+r**4/4 en cas de potentiel quartique de confinement

C  avec t->it la i-time methode sur NLSE donne les EDP a resoudre
C  Rho dU(t,X)/dt + Alfa LAPLACIEN U(t,X) - N(|U(t,X)|**2) U(t,X)
C                 + i OmegaZ (x dU(t,X)/dy - y dU(t,X)/dx) = -F(t,X)

C d'ou le SCHEMA SEMI-IMPLICITE MODIFIE en TEMPS avec Matrice GLOBALE nxn
C   n=0  V=v0      W=w0
C(7)m=0  V0n+1=Vn  W0n+1=Wn
C(8)(Rho/dt +Alfa LAPLACIEN - N(V0**2+W0**2) ) Vn+1m+1 =
C          Rho Vn/dt + Omega (x dWn+1m/dy - y dWn+1m/dx) 
C         +( N(Vn+1m**2+Wn+1m**2)-N(V0**2-W0**2) ) Vn+1m -Fr

C   (Rho/dt +Alfa LAPLACIEN - N(V0**2+W0**2) ) Wn+1m+1 =
C          Rho Wn/dt - Omega (x dVn+1m/dy - y dVn+1m/dx)
...
C [ KG(Rho,dt,V,N,V0)]
C si NORESO=1: FACTORISER [KG]= L D tL CROUT POUR RESOUDRE
C LE SYSTEME A CHAQUE ITERATION M ET POUR PLUSIEURS PAS DE TEMPS
C si NORESO=2: EMPLOYER LA METHODE DU GRADIENT CONJUGUE SIMPLE

C REMARQUE: LA MATRICE MG SERT A NORMALISER U A CHAQUE ITERATION
C
...
C          2 GRADIENT CONJUGUE SIMPLE STOCKAGE MORSE DE KG MG
...
C NBRDKG : NOMBRE DE REELS DOUBLE PRECISION DES MATRICES MG ET KG
...
C
      ...
      CHARACTER*16      TEXT
C
      DOUBLE PRECISION, allocatable, dimension(:) :: KG, MG
      INTEGER           IERKGALLOC, IERMGALLOC
      INTRINSIC         ALLOCATED

      REAL              RDeltaT, DTSTOC, TPSINI, TPSFIN, TSTOC
      DOUBLE PRECISION   DeltaT, NORMEL2U, NL2V, NL2W,
...
C     ALLOCATION DYNAMIQUE EN FORTRAN 90 DE LA MATRICE MG
      WRITE(IMPRIM,*)
      WRITE(IMPRIM,*) 'ALLOCATION DEMAND  of',NBRDKG,
     %                ' DOUBLE PRECISION of [MG] MATRIX'
...
ccc     %               ABS(WCAMAX(2)-WCAMIN(2)) )
      ...
      IF( LANGAG .EQ. 0 ) THEN
      WRITE(IMPRIM,*)'NLSEITIME: Max |Onde(t=',TEMPS,',NOEUD)|=',MODUMX0
      ELSE
       WRITE(IMPRIM,*)'NLSEITIME: Max |Wave(t=',TEMPS,',NODE)|=',MODUMX0
      ENDIF
...
C        KG MG PROFIL : TABLEAU POUR CALCULER LA NORME L2
         LOAUX = NBNOMA
         CALL TNMCDC( 'REEL2', LOAUX, MNAUX1 )

      ELSE
C
C        KG MG MORSE : LES 3 TABLEAUX AUXILIAIRES du SOUS PROGRAMME Gcaxbk
...
C        LE NOMBRE D'ITERATIONS DU GC MIS A JOUR APRES la m-CONVERGENCE
      ...
      NBL2TG  = 0
C
C     TEMPS DE CALCUL DE DECLARATION DE MG & KG
      CPU1   = DINFO( 'CPU' )
      CPUPKG = CPU1 - CPU0
      CPU0   = CPU1
C
...
C     [KG] = [Rho/dt + Alfa LAPLACIEN - N(|U0|**2)
...
C     COPIE DE UG00(NBNOMA,2) DANS UGtm(NBNOMA,2)
      ...
      CALL NLSITMGKG( PENALI, NDIM,   MNXYZN,
     %                NBTYEL, MNNPEF, NDPGST,
     %                MNTPOB, MNTAUX, NUMIOB, NUMAOB, MNDOEL,
     %                MNTHER, MNTAEL, MNX,
     %                NORESO, NCODSK, NBRDKG, MNLPLI, MNLPCO, MG, KG,
     %                NBPTAF, IERR  )
...
10050    FORMAT('NOMBRE DE CALCUL DE KG et MG=',I3)
      ELSE
         WRITE(IMPRIM,20050) NBCAKG
20050    FORMAT('KG and MG COMPUTATION NUMBER=',I3)
      ENDIF

      call affvect( 'MG SANS CL', 20,     MG )
      call afl1ve(  'MG SANS CL', NBRDKG, MG )

      call affvect( 'KG SANS CL', 20,     KG )
      call afl1ve(  'KG SANS CL', NBRDKG, KG )

      IF( NBCAKG .EQ. 1 .AND. NBCABG .EQ. 0 ) THEN

C        CALCUL DE LA NORME L2 DE L'ONDE UG00 AU TEMPS t0 AVEC [MG]
C        ( MG * VECTEUR, VECTEUR ) => CARRE DE LA NORME L2 de VECTEUR
C        ------------------------------------------------------------
         CALL ENERGY( NBNOMA, 1,
     %                NORESO, NCODSK, MCN(MNLPLI), MCN(MNLPCO), MG,
     %                MCN(MNUG00), MCN(MNAUX1),
     %                NL2V )
         CALL ENERGY( NBNOMA,   1,
     %                NORESO, NCODSK, MCN(MNLPLI), MCN(MNLPCO), MG,
     %                MCN(MNUG00+MOREE2*NBNOMA), MCN(MNAUX1),
     %                NL2W )
         NORMEL2U = SQRT( NL2V + NL2W )
         IF( LANGAG .EQ. 0 ) THEN
            WRITE(IMPRIM,*) 'NORME L2 DE L''ONDE UG00=',NORMEL2U,
     %                      ' au temps',TEMPS
         ELSE
         WRITE(IMPRIM,*) 'WAVE L2 UG00 NORM =',NORMEL2U,' at time',TEMPS
         ENDIF

C        UG00 <- UG00 / NORMEL2U   => ||UG00||L2 = 1
C        -------------------------------------------
         D = 1D0 / NORMEL2U
         MN0 = ( MNUG00 - 1 ) / MOREE2
         DO I=1,NTDL
            DMCN(MN0+I) = DMCN(MN0+I) * D
         ENDDO

         GOTO 50
C        POUR RECALCULER KG AVEC |UG00|L2=1
...
C           MATRICE NON INVERSIBLE
      ...
      ENDIF
C
C     SOMME DES TEMPS DE CALCUL DE CONSTRUCTION DE MG & KG
      CPU1   = DINFO( 'CPU' )
...
C
C     KG = [K(Alfa,g,V,Beta)] MATRICE GLOBALE NBNOMA x NBNOMA
C
C     ##################################################################
C     ##                                                              ##
...
10100 FORMAT(/'Au TEMPS',G14.6,'  PAS de TEMPS=',G15.7,
     %'Nb PAS TEMPS=',I10,
     %' ITERm=',I3,
     %'  Total ITERm=',I10,' -> CALCUL de l''ONDE COMPLEXE'/140('*'))
20100 FORMAT(/'At TIME',G14.6,' TIME STEP=',G15.7,
     %' TIME STEP Nb=',I10,
     %' ITERm=',I3,
     %'  Total ITERm=',I10,' -> COMPUTATION of the COMPLEX WAVE'/
     %140('*'))
C
C     GENERATION DU SECOND MEMBRE PARTIE RELLE BG=Fn+1,m+1 AU TEMPS tn+1
C     TESTNL=9
C     REAL PART=Rho/dt Vn + OmegaZ ( x dWn+1m/dy - y dWn+1m/dx )
C              -Beta (Vn+1m**2+Wn+1m**2-V0**2-W0**2) Vn+1m   + Fr
C     IMAG PART=Rho/dt Wn - OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )
C              -Beta (Vn+1m**2+Wn+1m**2-V0**2-W0**2) Wn+1m+1 + Fi
C     ==================================================================
      MNTHET = MNUGtm
      CALL NLSITBG( DeltaT, PENALI, NDIM,   MNXYZN, NBNOMA,
     %              NBTYEL, MNNPEF, NDPGST,
     %              MNTPOB, MNTAUX, NUMIOB, NUMAOB, MNDOEL,
...
C     NETTOYAGE DES PETITES VALEURS DE UGtm1
      ...
      ENDDO
C
C     CALCUL DE LA NORME L2 DE L'ONDE AU TEMPS tn+1 m+1
C     -------------------------------------------------
C     ( MG * VECTEUR, VECTEUR ) => CARRE DE LA NORME L2 de VECTEUR
      CALL ENERGY( NBNOMA,   1,
     %             NORESO, NCODSK, MCN(MNLPLI), MCN(MNLPCO), MG,
     %             MCN(MNUGtm1), MCN(MNAUX1),
     %             NL2V )
      CALL ENERGY( NBNOMA,   1,
     %             NORESO, NCODSK, MCN(MNLPLI), MCN(MNLPCO), MG,
     %             MCN(MNUGtm1+MOREE2*NBNOMA), MCN(MNAUX1),
     %             NL2W )
      NORMEL2U = SQRT( NL2V + NL2W )
      IF( LANGAG .EQ. 0 ) THEN
         WRITE(IMPRIM,10101) TEMPS,ITERm,NORMEL2U
10101    FORMAT('Au TEMPS ',G14.6,' ITER m=',I3,
     %          ' NORME L2 de l''ONDE=',G14.6)
      ELSE
         WRITE(IMPRIM,20101) TEMPS,ITERm,NORMEL2U
20101    FORMAT('At TIME ',G14.6,' ITER m=',I3,
     %          ' WAVE L2 NORM =',G14.6)
      ENDIF
C
C     U <- U / NORMEL2U   => ||U||L2 = 1
C     ----------------------------------
      D   = 1D0 / NORMEL2U
      MN1 = ( MNUGtm1 - 1 ) / MOREE2
      DO I=1,NTDL
         DMCN(MN1+I) = DMCN(MN1+I) * D
      ENDDO
...
C        LA NORME L2 DE U AVANT NORMALISATION A 1
         I = NUDCNB( KNOM )
         KNOM(I+1:I+7) = ' |U|L2='
         WRITE( TEXT, '(G14.6)' ) NORMEL2U
         CALL TEXTSB( TEXT, L )
         I = NUDCNB( KNOM )
         KNOM(I+1:I+L) = TEXT(1:L)
...
C        SAUVEGARDE DES RESULTATS ACTUELS UGtm1 et SORTIE
      ...
      ENDIF
C
ccc 150  IF( ITERm .GT. 5 .AND. Testm .GT. 1D-2 ) THEN
 150  IF( ITERm .GE. 8 ) THEN
C
...
C
ccc         IF( Testm .LT. 5D-4 .AND. ITERm .LE. 1 ) THEN
         IF( Testm .LT. 2D-4 .AND. ITERm .LE. 1 ) THEN
C
C           NON: POUR ACCELERER: RECONSTRUCTION DE KG avec un PAS DE TEMPS*1.5
C           ==================================================================
C           LE NOUVEAU TEMPS INITIAL EST tn
            TEMPSINI = TEMPS
...
C
C           RECONSTRUCTION DE KG avec U0=(Un+1,m+1) TOUS LES 5 PAS DE TEMPS
C           ===============================================================
...
C
         ENDIF
C
         IF( NORMEL2U .LT. 0.9D0 .OR. NORMEL2U .GT. 1.1D0 ) THEN

C           UNE ITERATION DE PLUS AVEC NORMEL2U TROP ELOIGNE DE 1
            NBL2TG = NBL2TG + 1

            IF( NBL2TG .GE. 100 ) THEN

C              POUR ACCELERER: RECONSTRUCTION DE KG avec le MEME PAS DE TEMPS
C              ==============================================================
C              LE NOUVEAU TEMPS INITIAL EST tn
               TEMPSINI = TEMPS
C              SOLUTION INITIALE REMPLACEE PAR LA DERNIERE SOLUTION CALCULEE
               CALL TRTATD( MCN(MNUGtm1), MCN(MNUG00), NTDL )
               IF( LANGAG .EQ. 0 ) THEN
                  WRITE(IMPRIM,10200) DeltaT, DeltaT
               ELSE
                  WRITE(IMPRIM,20200) DeltaT, DeltaT
               ENDIF
               NBL2TG = 0
               GOTO 50
...
C        CROUT PROFIL
         ...
         ENDIF
19002 FORMAT(/'NLSEITIME: STOCKAGE MATRICE PROFIL + VECTEURS =',I15,
     %' MOTS'/ )
29002 FORMAT(/'NLSEITIME: SKYLINE MATRIX + VECTORS STORAGE =',I15,
     %' MEMORY WORDS')
      ELSE
         IF( LANGAG .EQ. 0 ) THEN
         ...
         ENDIF
19003 FORMAT(/'NLSEITIME: STOCKAGE MATRICE MORSE + VECTEURS=',I15,
     %' MOTS MEMOIRE'/ )
29003 FORMAT(/'NLSEITIME: CONDENSED MATRIX + VECTORS STORAGE=',I15,
     %' MEMORY WORDS')
      ENDIF
C
