      SUBROUTINE NSB2P1D( NUELEM, NONOEF, Omega,
     ...
     %                    BE )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT : NLSE: CALCUL DU SECOND MEMBRE BE SUR UN EF LAGRANGE
C -----              TRIANGLE 2P1D

C TESTNL=6: SEMI-IMPLICITE
C BE1 = Fr(tn+1) + Rho/dt (Wn+1m-Wn) -OmegaZ (x dWn+1m/dy - y dWn+1m/dx)
C                + ( N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2) ) Vn+1m         
C BE2 = Fi(tn+1) - Rho/dt (Vn+1m-Vn) +OmegaZ (x dVn+1m/dy - y dVn+1m/dx)
C                + ( N(V0**2-W0**2)-N(Vn+1m**2+Wn+1m**2) ) Wn+1m

C TESTNL=7: IMPLICITE
C BE1 = -Fr(tn+1) - (N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2)) Vn+1m
C       +Rho/dt Wn +OmegaZ ( x dWn+1m/dy - y dWn+1m/dx
C BE2 =  Fi(tn+1) + (N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2)) Wn+1m
C       +Rho/dt Vn +OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )

C TESTNL=9: Gross-Pitaevskii SEMI-IMPLICITE i time methode
C BE1 = Fr(tn+1) + Rho/dt Vn + OmegaZ ( x dWn+1m/dy - y dWn+1m/dx )
C       + ( -N(Vn+1m**2+Wn+1m**2)+N(V0**2-W0**2) ) Vn+1m
C BE2 = Fi(tn+1) + Rho/dt Wn + OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )
C       + ( -N(Vn+1m**2+Wn+1m**2)+N(V0**2-W0**2) ) Wn+1m
...
C
      ...
      DOUBLE PRECISION  S, D, DELTA, NLSECOEF, NLSECOEF0
      DOUBLE PRECISION  Rho, NLCOEF, FORCE(2), FIXA(2), DP(2,3),
     %                  F1dYF2dXV, F1dYF2dXW,
...
C        CALCUL de ( x dWn+1m/dy - y dWn+1m/dx )(bl)
         ...
         ENDDO
C        ( x dVn+1m/dy - y dVn+1m/dx )(bl)
         F1dYF2dXV = F1dYF2dXV / DELTA
C        ( x dWn+1m/dy - y dWn+1m/dx )(bl)
         F1dYF2dXW = F1dYF2dXW / DELTA
C
C        DENSITE DE MASSE
C        ----------------
         ...
         ENDIF
C
C        NLCOEF COEFFICIENT DU TERME NON LINEAIRE AU TEMPS ACTUEL
C       -NLCOEF COEFFICIENT DU TERME NON LINEAIRE AU TEMPS INITIAL
C        ( ex: NLCOEF (V**2+W**2) - NLCOEF (V0**2+W0**2) )
C        ------------------------------------------------------
         MN = LTDESU(LPCOET,JEU,NOOBSF)
         IF( MN .GT. 0 ) THEN
            CALL RENLSE( 3, NOOBSF, 3, XYZ, TEMPS, VEFtm(I),WEFtm(I),MN,
     %                   NLSECOEF )
            CALL RENLSE( 3, NOOBSF, 3,XYZ,TEMPSINI,VEFtn(I),WEFtn(I),MN,
     %                   NLSECOEF0 )
            NLCOEF = NLSECOEF - NLSECOEF0
         ELSE
            NLCOEF = 0D0
         ENDIF
C
...
C        CALCUL DES 2 COEFFICIENTS I DU VECTEUR ELEMENTAIRE BE(3,2)
C        ----------------------------------------------------------
         IF( TESTNL .EQ. 6 ) THEN
...
C              + ( N(V0**2+W0**2) - N(Vn+1m**2+Wn+1m**2) ) Vn+1m
            BE(I,1) = S * ( FORCE(1) + Rho/DeltaT * (WEFtm(I)-WEFtn(I))
     %                    - Omega(1) * F1dYF2dXW
     %                    - NLCOEF * VEFtm(I) )
C
C           BE2: Fi(tn+1)
...
C              + ( N(V0**2-W0**2) - N(Vn+1m**2+Wn+1m**2) ) Wn+1m
            BE(I,2) = S * ( FORCE(2) - Rho/DeltaT * (VEFtm(I)-VEFtn(I))
     %                    + Omega(1) * F1dYF2dXV
     %                    - NLCOEF * WEFtm(I) )
C
...
C           BE1:  -Fr(tn+1) - (N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2)) Vn+1m
C                 +Rho/dt Wn +OmegaZ ( x dWn+1m/dy - y dWn+1m/dx )
            BE(I,1) = S * ( NLCOEF * VEFtm(I)  - FORCE(1)
     %                     +Rho/Deltat * WEFtn(I) +Omega(1) *F1dYF2dXW )

C           BE2:  Fi(tn+1) + (N(V0**2+W0**2)-N(Vn+1m**2+Wn+1m**2)) Wn+1m
C                +Rho/dt Vn +OmegaZ ( x dVn+1m/dy - y dVn+1m/dx )
            BE(I,2) = S * ( FORCE(2)  -NLCOEF * WEFtm(I) 
     %                     +Rho/Deltat * VEFtn(I) +Omega(1) *F1dYF2dXV )

         ELSE IF( TESTNL .EQ. 9 ) THEN

C           BE1: Fr(tn+1) + Rho/dt Vn -(N(Vn+1m**2+Wn+1m**2)+N(V0**2-W0**2)) Vn+1m
C                         + Omega ( x dWn+1m/dy - y dWn+1m/dx )
            BE(I,1) = S * ( FORCE(1) + Rho/DeltaT * VEFtn(I)
     %                    - NLCOEF * VEFtm(I)
     %                    + Omega(1) * F1dYF2dXW )

C           BE2: FI(tn+1) + Rho/dt Wn -(N(Vn+1m**2+Wn+1m**2)+N(-V0**2-W0**2)) Wn+1m
C                         - Omega ( x dVn+1m/dy - y dVn+1m/dx )
            BE(I,2) = S * ( FORCE(2) + Rho/DeltaT * WEFtn(I)
     %                    - NLCOEF * WEFtm(I)
     %                    - Omega(1) * F1dYF2dXV )
