      SUBROUTINE ECLAXI( NUELEM,NBELEM,NBPOLY,NUNDEL,NPI,NDSM,NDSMCI,
     %                   POLY,DP,F1,F2,
...
C ----- POUR LES ELEMENTS FINIS LAGRANGE DE DEGRE 1 OU 2 AXISYMETRIQUES
C
C ENTREES :
C ---------
...
C F1,F2  : RAYON ET COTE DES NPI POINTS D INTEGRATION
C
...
C STRELT : TABLEAU AUXILIAIRE (4,2*NBPOLY)
C CONINI : TABLEAU AUXILIAIRE (NDSM,4)
C
...
C
         ...
         ED1 ( 4 ) = ELAS(9) * D
C
C        ED1 * P + ED2 (1 ET 2 ) * DP => STRELT(1 A NBPOLY)
C        --------------------------------------------------
         CALL AB0D(4,1,NBPOLY,ED1,POLY(1,L),STRELT(1,1))
         CALL AB1D(4,2,NBPOLY,ED2,DP(1,1,L),STRELT(1,1))
C
C        ED2( 2 ET 3 ) * DP => STRELT( 1 + NBPOLY A 2 * NBPOLY)
C        ------------------------------------------------------
         CALL AB0D(4,2,NBPOLY,ED2(1,2),DP(1,1,L),STRELT(1,1+NBPOLY))
C
C        CONTRIBUTION DES DEPLACEMENTS AU TENSEUR DES CONTRAINTES
    ...
    9    CONTINUE
C
C        CONTRAINTES THERMIQUES -(E) (DILATA) (P) (TEMP - TEMP0)
C        -------------------------------------------------------
         IF( MNTEMP .GT. 0 ) THEN
...
C              CALCUL DE ED1 = - (E) (DILATA)
               D = - DILATA
               ED1(1) = D * ( ELAS(1) + ELAS(2) + ELAS(4) )
               ED1(2) = D * ( ELAS(2) + ELAS(3) + ELAS(5) )
               ED1(3) = D * ( ELAS(4) + ELAS(5) + ELAS(6) )
               ED1(4) = D * ( ELAS(7) + ELAS(8) + ELAS(9) )
C
C              CALCUL DE STRELT = (ED1) (P)
               CALL AB0D(4,1,NBPOLY,ED1,POLY(1,L),STRELT(1,1))
               DO 19 K=1,4
                  DO 18 J=1,NBPOLY
                     DO 17 M=1,NDSM
