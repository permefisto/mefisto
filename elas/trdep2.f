...
C           LE NUMERO DES 2 SOMMETS DE L'ARETE I DE L'EF K
     ...
     %          + AMPLID * DEPLAC(NDIMES*(NS1-1)+1,NCAS) )
            YS1 = REAL( XYZNOE(2,NS1)
     %          + AMPLID * DEPLAC(NDIMES*(NS1-1)+2,NCAS) )
            NS2 = NUNDEL( K, NOSOAR(2,I) )
            XS2 = REAL( XYZNOE(1,NS2)
     %          + AMPLID * DEPLAC(NDIMES*(NS2-1)+1,NCAS) )
            YS2 = REAL( XYZNOE(2,NS2)
     %          + AMPLID * DEPLAC(NDIMES*(NS2-1)+2,NCAS) )
C
            IF( NBNOAR(I) .LE. 0 ) THEN
...
C              TRACE DU SOMMET1 AU MILIEU DE L'ARETE I DE L'EF K
               NM = NUNDEL( K, NONOAR(1,I) )
               XM = REAL( XYZNOE(1,NM)
     %            + AMPLID * DEPLAC(NDIMES*(NM-1)+1,NCAS) )
               YM = REAL( XYZNOE(2,NM)
     %            + AMPLID * DEPLAC(NDIMES*(NM-1)+2,NCAS) )
