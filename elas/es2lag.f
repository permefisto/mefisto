      SUBROUTINE ES2LAG( X,      PENALI, NBSOMT, NBCOTE, NDSM,
     ...
     %                   BE )
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C BUT :    CALCUL DES SECONDS MEMBRES DES ELEMENTS LAGRANGE 2D
C -----    ISOPARAMETRIQUES DE DEGRE 1 OU 2 SUR UN TRIANGLE OU QUADRANGLE
C
C ENTREES:
C --------
...
C MODIFS : FERNANDES-VARELA ANALYSE NUMERIQUE UPMC PARIS       MARS 1998
...
C           LA VALEUR DES EFFORTS SURFACIQUES EN CE POINT
            CALL REFORC( 3,NOOBSF,2,
     %                   F1(L),F2(L),0D0, 0D0,0D0,0D0,
...
C           LA VALEUR DES CONTRAINTES INITIALES EN CE POINT
     ...
     %                   LTDESU(LPCOIN,NOOBSF), FGAMMA )
            DO 85 N=1,NDSM
C              FGAMMA =  S11 S22 S12   DEVIENT S11 S12 S12 S22
               A(1) = FGAMMA(N,1)
               A(2) = FGAMMA(N,3)
               A(3) = FGAMMA(N,3)
               A(4) = FGAMMA(N,2)
C
C              CALCUL DE G1(NBPOLY,1) = T(DP) T(A(1),A(2))
               CALL TAB0D(NBPOLY,2,1,DP(1,1,L),A(1),G1(1,1))
C
C              CALCUL DE G1(NBPOLY,2) = T(DP) T(A(3),A(4))
...
C                    LA COMPOSANTE DES CONTRAINTES INITIALES
                     BE(N,I1) = BE(N,I1) - POIDEL(L) * G1(I,J)
   75             CONTINUE
   80          CONTINUE
   85       CONTINUE
   90    CONTINUE
...
C             CALCUL DE A=T(D)*E*DILATA*POIDS*JACOBIEN
              DD   = POIDEL(L) * DILATA
              A(1) = ( ELAS(1)+ELAS(2) ) * DD
              A(2) = ( ELAS(4)+ELAS(5) ) * DD
              A(3) =  A(2)
              A(4) = ( ELAS(2)+ELAS(3) ) * DD
C
C             CALCUL DE G1(NBPOLY,1) = T(DP) T(A(1),A(2))
              CALL TAB0D(NBPOLY,2,1,DP(1,1,L),A(1),G1(1,1))
C
